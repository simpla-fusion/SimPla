...
static std::string GetSimPlaMajorFileName(const char *path)
{
...
// ----------------------------------------------------------------------------
//                       Class vtkAMRSimPlaReaderBlock (begin)
// ----------------------------------------------------------------------------


class vtkAMRSimPlaReaderBlock
{
public:
    vtkAMRSimPlaReaderBlock() { this->Init(); }

    ~vtkAMRSimPlaReaderBlock() { this->Init(); }

    vtkAMRSimPlaReaderBlock(const vtkAMRSimPlaReaderBlock &other) { this->DeepCopy(&other); }

    vtkAMRSimPlaReaderBlock &operator=(const vtkAMRSimPlaReaderBlock &other)
    {
        this->DeepCopy(&other);
        return *this;
    }
    ...
    void DeepCopy(const vtkAMRSimPlaReaderBlock *other);

    void GetParentWiseIds(std::vector<vtkAMRSimPlaReaderBlock> &blocks);

    void GetLevelBasedIds(std::vector<vtkAMRSimPlaReaderBlock> &blocks);
};


// ----------------------------------------------------------------------------
//                       Class vtkAMRSimPlaReaderBlock ( end )
// ----------------------------------------------------------------------------

//------------------------------------------------------------------------------
void vtkAMRSimPlaReaderBlock::Init()
{
    this->BlockFileName = "";
...
//------------------------------------------------------------------------------
void vtkAMRSimPlaReaderBlock::DeepCopy(const vtkAMRSimPlaReaderBlock *other)
{
...
void vtkAMRSimPlaReaderBlock::GetParentWiseIds
        (std::vector<vtkAMRSimPlaReaderBlock> &blocks)
{
    if (this->ParentId != 0)
    {
        ...
        // beginning position
        vtkAMRSimPlaReaderBlock &parent = blocks[this->ParentId];
    ...
    else
    {
        ...
        vtkAMRSimPlaReaderBlock &block0 = blocks[0];
...
//------------------------------------------------------------------------------
void vtkAMRSimPlaReaderBlock::GetLevelBasedIds
        (std::vector<vtkAMRSimPlaReaderBlock> &blocks)
{
...
// ----------------------------------------------------------------------------
//                     Class  vtkAMRSimPlaReaderInternal (begin)
// ----------------------------------------------------------------------------



class vtkAMRSimPlaReader::pimpl_s
{
public:
    pimpl_s();
...
//  vtkAMRSimPlaReader * TheReader;
    ...
    std::string GetBaseDirectory(const char *path) { return GetSimPlaDirectory(path); };


};

vtkAMRSimPlaReader::pimpl_s::pimpl_s()
{
    this->Init();
}
...
//------------------------------------------------------------------------------
void vtkAMRSimPlaReader::pimpl_s::Init()
{
    this->DataTime = 0.0;
    this->FileName = NULL;
    //this->TheReader  = NULL;
...
//------------------------------------------------------------------------------
void vtkAMRSimPlaReader::pimpl_s::ReleaseDataArray()
{
    if (this->DataArray)
    {
...
//------------------------------------------------------------------------------
int vtkAMRSimPlaReader::pimpl_s::GetBlockAttribute(
...
//------------------------------------------------------------------------------
int vtkAMRSimPlaReader::pimpl_s::LoadAttribute(const char *atribute, int blockIdx)
{
    // TODO: implement this
    ...
    // this->m_pimpl_->Blocks includes a pseudo block --- the root as block #0
    blockIdx++;
...
void vtkAMRSimPlaReader::pimpl_s::ReadBlockStructures()
{
    ...
    vtkAMRSimPlaReaderBlock block0;
    block0.Index = 0;
    block0.Level = -1;
    block0.ParentId = -1;
    ...
    while (stream)
    {
        ...
        if (theStr == "Grid")
        {
            vtkAMRSimPlaReaderBlock tmpBlk;
...
//      std::cout.flush();
            tmpBlk.BlockFileName =
                    this->DirectoryName + "/" + GetSimPlaMajorFileName(szName.c_str());
...
void vtkAMRSimPlaReader::pimpl_s::ReadGeneralParameters()
{
...
void vtkAMRSimPlaReader::pimpl_s::DetermineRootBoundingBox()
{
    vtkAMRSimPlaReaderBlock &block0 = this->Blocks[0];
...
void vtkAMRSimPlaReader::pimpl_s::GetAttributeNames()
{
    ...
    {
        vtkAMRSimPlaReaderBlock &tmpBlock = this->Blocks[i];
...
// (such as the one adopted in cosmological datasets) causes this SimPla reader,
...
void vtkAMRSimPlaReader::pimpl_s::CheckAttributeNames()
{
    // number of cells of the reference block
    vtkAMRSimPlaReaderBlock &
...
// ----------------------------------------------------------------------------
// get the meta data
void vtkAMRSimPlaReader::pimpl_s::ReadMetaData()
{
...
vtkStandardNewMacro(vtkAMRSimPlaReader);


void vtkAMRSimPlaReader::ComputeStats(std::vector<int> &numBlocks, double min[3])
{
    min[0] = min[1] = min[2] = std::numeric_limits<double>::max();
    numBlocks.resize(this->m_pimpl_->NumberOfLevels, 0);

    for (int i = 0; i < this->m_pimpl_->NumberOfBlocks; ++i)
    {
        vtkAMRSimPlaReaderBlock &theBlock = this->m_pimpl_->Blocks[i + 1];
...
//-----------------------------------------------------------------------------
vtkAMRSimPlaReader::vtkAMRSimPlaReader() : m_pimpl_(new pimpl_s)
{
    this->IsReady = false;
    this->Initialize();
    this->ConvertToCGS = 1;
...
//-----------------------------------------------------------------------------
vtkAMRSimPlaReader::~vtkAMRSimPlaReader()
{
    this->m_pimpl_.release();
...
//-----------------------------------------------------------------------------
void vtkAMRSimPlaReader::PrintSelf(std::ostream &os, vtkIndent indent)
{
    this->Superclass::PrintSelf(os, indent);
}

//-----------------------------------------------------------------------------
int vtkAMRSimPlaReader::GetIndexFromArrayName(std::string arrayName)
{
    char stringIdx[2];
...
//-----------------------------------------------------------------------------
double vtkAMRSimPlaReader::GetConversionFactor(const std::string name)
{
...
//-----------------------------------------------------------------------------
void vtkAMRSimPlaReader::ParseLabel(
...
//-----------------------------------------------------------------------------
void vtkAMRSimPlaReader::ParseCFactor(
...
//-----------------------------------------------------------------------------
void vtkAMRSimPlaReader::ParseConversionFactors()
{
...
//-----------------------------------------------------------------------------
void vtkAMRSimPlaReader::SetFileName(const char *fileName)
{
    assert("pre: Internal SimPla AMR Reader is NULL" && (this->m_pimpl_ != NULL));
        ...
        ((this->FileName == NULL) || (strcmp(fileName, this->FileName))))
    {
            ...
            tempName.substr(tempName.length() - hExtName.length()) == hExtName)
        {
            ...
            this->m_pimpl_->BoundaryFileName = this->m_pimpl_->MajorFileName + bExtName;
        }
                 ...
                 tempName.substr(tempName.length() - bExtName.length()) == bExtName)
        {
            ...
            this->m_pimpl_->HierarchyFileName = this->m_pimpl_->MajorFileName + hExtName;
        }
        else
        {
    ...
    {
        this->BlockMap.clear();
        this->m_pimpl_->Blocks.clear();
        this->m_pimpl_->NumberOfBlocks = 0;
        ...
        if (this->FileName != NULL)
        {
            delete[] this->FileName;
            this->FileName = NULL;
            this->m_pimpl_->SetFileName(NULL);
        }
...
//-----------------------------------------------------------------------------
void vtkAMRSimPlaReader::ReadMetaData()
{
    assert("pre: Internal SimPla Reader is NULL" && (this->m_pimpl_ != NULL));
...
//-----------------------------------------------------------------------------
int vtkAMRSimPlaReader::GetBlockLevel(const int blockIdx)
{
    assert("pre: Internal SimPla Reader is NULL" && (this->m_pimpl_ != NULL));
    ...
    if (blockIdx < 0 || blockIdx >= this->m_pimpl_->NumberOfBlocks)
    {
        vtkErrorMacro("Block Index (" << blockIdx << ") is out-of-bounds!");
        return (-1);
    }
    return (this->m_pimpl_->Blocks[blockIdx + 1].Level);
}

//-----------------------------------------------------------------------------
int vtkAMRSimPlaReader::GetNumberOfBlocks()
{
    assert("pre: Internal SimPla Reader is NULL" && (this->m_pimpl_ != NULL));
    if (!this->IsReady)
    {
    ...
    this->m_pimpl_->ReadMetaData();
    return (this->m_pimpl_->NumberOfBlocks);
}

//-----------------------------------------------------------------------------
int vtkAMRSimPlaReader::GetNumberOfLevels()
{
    assert("pre: Internal SimPla Reader is NULL" && (this->m_pimpl_ != NULL));
    if (!this->IsReady)
    {
    ...
    this->m_pimpl_->ReadMetaData();
    return (this->m_pimpl_->NumberOfLevels);
}

//-----------------------------------------------------------------------------
int vtkAMRSimPlaReader::FillMetaData()
{
    assert("pre: Internal SimPla Reader is NULL" && (this->m_pimpl_ != NULL));
    ...
    this->ComputeStats(blocksPerLevel, origin);
    ...
    std::vector<int> b2level(this->m_pimpl_->NumberOfLevels + 1, 0);
    for (int block = 0; block < this->m_pimpl_->NumberOfBlocks; ++block)
    {
        vtkAMRSimPlaReaderBlock &theBlock = this->m_pimpl_->Blocks[block + 1];
    ...
    this->Metadata->GetInformation()->Set(vtkDataObject::DATA_TIME_STEP(), this->m_pimpl_->DataTime);
    return (1);
}

//-----------------------------------------------------------------------------
vtkUniformGrid *vtkAMRSimPlaReader::GetAMRGrid(const int blockIdx)
{
    assert("pre: Internal SimPla Reader is NULL" && (this->m_pimpl_ != NULL));
    ...
    // this->m_pimpl_->Blocks includes a pseudo block --- the root as block #0
    vtkAMRSimPlaReaderBlock &theBlock = this->m_pimpl_->Blocks[blockIdx + 1];
    double blockMin[3];
    double blockMax[3];
    double spacings[3];
...
//-----------------------------------------------------------------------------
void vtkAMRSimPlaReader::SetUpDataArraySelections()
{
    assert("pre: Internal SimPla Reader is NULL" && (this->m_pimpl_ != NULL));
    this->m_pimpl_->ReadMetaData();
    this->m_pimpl_->GetAttributeNames();

    int numAttrs = static_cast< int >(this->m_pimpl_->BlockAttributeNames.size());
    for (int i = 0; i < numAttrs; i++)
    {
        this->CellDataArraySelection->AddArray(
                this->m_pimpl_->BlockAttributeNames[i].c_str());
