...
void
setupHierarchy(boost::shared_ptr<tbox::Database> main_input_db,
...
//         }
        ...
        setupHierarchy(input_db, dim, geom, mblk_patch_hierarchy);

        MblkLinAdv *linear_advection_model = new MblkLinAdv("MblkLinAdv", dim,
                                                            input_db, geom);

        boost::shared_ptr<MblkHyperbolicLevelIntegrator> mblk_hyp_level_integrator(
                new MblkHyperbolicLevelIntegrator("HyperbolicLevelIntegrator",
                                                  dim, input_db->getDatabase("HyperbolicLevelIntegrator"),
                                                  linear_advection_model, mblk_patch_hierarchy,
        ...
        boost::shared_ptr<algs::TimeRefinementIntegrator> time_integrator(
                new algs::TimeRefinementIntegrator("TimeRefinementIntegrator",
                                                   ...
                                                   mblk_patch_hierarchy, mblk_hyp_level_integrator,
        ...
        boost::shared_ptr<appu::VisItDataWriter> visit_data_writer(
                new appu::VisItDataWriter(dim, "MblkLinAdv VisIt Writer",
...
void setupHierarchy(boost::shared_ptr<tbox::Database> main_input_db,
                    const tbox::Dimension &dim,
                    boost::shared_ptr<hier::BaseGridGeometry> &geometry,
                    boost::shared_ptr<hier::PatchHierarchy> &mblk_hierarchy)
{
    ...
    {
        geometry.reset(
                new geom::GridGeometry(dim, geom_name,
                                       main_input_db->getDatabase(geom_name)));
    } else
    {
        TBOX_ERROR(
                "main::setupHierarchy(): could not find entry `" << geom_name << "' in input.");
    }
