...
CUDA_DEVICE
void cache_gather(Real *v, Real const f[CACHE_SIZE], Real const *r0, const Real *r1)
{
...
CUDA_GLOBAL void
spBorisYeePush_kernel(struct spPage *pg, Real cmr, double dt,
                      const Real tE[3][CACHE_SIZE],
                      const Real tB[3][CACHE_SIZE],
                      const Real *inv_dx)
{
    int i = threadIdx.x;
    ...
    if (pg->tag & (0x1 << i) != 0)
    {
        ...
        struct boris_point_s *p = (struct boris_point_s *) (pg->data + i * sizeof(struct boris_point_s));
        Real E[3], B[3];
...
spBorisYeePush(struct spPage *pg, Real cmr, double dt,
               const Real *E, const Real *B,
               size_type const *i_self_,
               size_type const *i_lower_,
               size_type const *i_upper_,
               const Real *inv_dx)
{

//    size_t els_size_in_byte = sizeof(struct boris_point_s);
    Real tE[3][CACHE_SIZE], tB[3][CACHE_SIZE];
    ...
    while (pg != 0x0)
    {
        ...
        spBorisYeePush_kernel<<<numBlocks,threadsPerBlock >>> (pg, cmr, dt, tE, tB, inv_dx);
        /* @formatter:on*/
