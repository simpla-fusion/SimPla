...
void create_scenario(simulation::Context *ctx, toolbox::ConfigParser const &options)
{
    ...
    center_domain->mesh()->dimensions(options["MeshView"]["Dimensions"].template as<index_tuple>(size_tuple{20, 20, 1}));
    center_domain->mesh()->ghost_width(options["MeshView"]["GhostWidth"].template as<index_tuple>(size_tuple{2, 2, 2}));
    center_domain->mesh()->box(options["MeshView"]["MeshBlock"].template as<box_type>(box_type{{{0, 0, 0}, {1, 1, 1}}}));
