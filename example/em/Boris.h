...
void spBorisPushOne(struct boris_point_s const *p, struct boris_point_s *p_next,
		Real cmr, Real dt, Real const E[3], Real const B[3], const Real *inv_dx)
{
...
Real spBorisGetRho(struct boris_point_s const *p)
{

	return p->f * p->w;
}

Real spBorisGetJ(struct boris_point_s const *p, int n)
{
	return p->f * p->w * p->v[n];
}

Real spBorisGetE(struct boris_point_s const *p)
{
	return 0.5 * p->f * p->w
