...
INLINE_PREFIX void
spBorisPushOne(struct boris_point_s const *p, struct boris_point_s *p_next, Real cmr, double dt,
               Real const E[3], Real const B[3], const Real *inv_dx)
{
...
{
    return 0.5 * p->f * p->w * (p->v[0] * p->v[0] + p->v[1] * p->v[1] + p->v[2] * p->v[2]);
}
