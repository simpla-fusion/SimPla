...
USE_CASE(model,"Model")
{
	...
	std::vector<coordinates_type> pp, p0, p1, p2, p3;
...
//	else
	...
	std::map<id_type,
			std::tuple<Real, typename std::vector<coordinates_type>::iterator,
					typename std::vector<coordinates_type>::iterator, Vec3>> vertices_close_to_polylines;

	select_points_near_to_polylines(domain0, pp.begin(), pp.end(), mesh->dx(),
...
//			1);
	...
	domain0.for_each_coordinates([&](coordinates_type const &x)
	{
		p0.push_back(x);
	});

	domain1.for_each_coordinates([&](coordinates_type const &x)
	{
		p1.push_back(x);
	});

	domain2.for_each_coordinates([&](coordinates_type const &x)
	{
		p2.push_back(x);
	});

	domain3.for_each_coordinates([&](coordinates_type const &x)
	{
		p3.push_back(x);
	});
