...
struct EMTokamak {
    ...
    std::pair<typename std::map<std::string, fluid_s>::iterator, bool> add_particle(std::string const &name, Real mass,
                                                                                    Real charge) {
...
template <typename TP, typename TDict, typename TRange>
std::shared_ptr<particle::ParticleBase> EMTokamak::create_particle(std::string const &key, TDict const &dict,
                                                                   TRange const &r0) {
...
void EMTokamak::initialize(int argc, char **argv) {
...

    {
        ...
        if (options["MeshView"]["Geometry"]["Box"].as(&box)) {
        ...
        auto dims = options["MeshView"]["Geometry"]["Topology"]["Dimensions"].template as<nTuple<size_t, 3>>();

        m.dimensions(dims);
        m.ghost_width(index_tuple({2, 2, 0}));

        m.dt(options["MeshView"]["dt"].template as<Real>(1.0));
    }
    ...
    parallel::parallel_for(m.template range<FACE>(), [&](range_type const &r) {
        for (auto const &s : r) { B0.assign(s, geqdsk.B(m.point(s))); }
    });
    ...
    parallel::parallel_for(m.template range<VERTEX>(), [&](range_type const &r) {
        for (auto const &s : r) {
            auto x = m.point(s);
...

    {
        ...
        if (dict) {
            model::create_id_set(m, m.template make_range<EDGE>(m.index_box(dict["Box"].template as<box_type>())),
                                 &J_src);
    ...
    try {
        ...
        for (auto const &dict : ps) {
            ...
            if (engine == "Gyro") {
                particle_sp[key] =
                    create_particle<particle::GyroParticle<mesh_type>>(key, dict.second, plasma_region_volume.range());
    ...
    } catch (std::exception const &error) { RUNTIME_ERROR << "Load particle error!" << error.what() << std::endl; }
...
int main(int argc, char **argv) {
    ...
    try {
        ...
        options.init(argc, argv);
    } catch (std::exception const &error) { RUNTIME_ERROR << "Initial error" << error.what() << std::endl; }
    ...
    } catch (std::exception const &error) { RUNTIME_ERROR << "Context Intialize error!" << error.what() << std::endl; }
    ...
    while (count < num_of_steps) {
        ctx->next_time_step();
        if (count % check_point == 0) ctx->check_point();
        ++count;
    }
    ctx->tear_down();
