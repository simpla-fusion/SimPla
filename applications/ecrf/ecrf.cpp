...
int main(int argc, char **argv)
{
	...
	DEFINE_FIELDS(Real, Grid);

	TR1::shared_ptr<Grid> grid(new Grid);

	TR1::shared_ptr<Context> ctx(new Context);

	grid->Initialize(1 / 200.0, xmin, xmax, dims);
...
//	ctx->AddSolver("DeltaF", new PICEngine<DeltaF, Grid>(ctx));
	...
	typename pic::PICEngine<pic::FullF, Grid>::Holder pic_full(
			pic::PICEngine<pic::FullF, Grid>::Create(ctx));

	pic_full->set_property("electron", 1.0, -1.0, T, 200);

	ctx->functions.push_back(
			TR1::bind(&pic::PICEngine<pic::FullF, Grid>::Process, pic_full));

	ctx->functions.push_back(TR1::bind(&Maxwell<Real, Grid>, ctx));

	ctx->functions.push_back(
			TR1::bind(&PML<Real, Grid>::Process, new PML<Real, Grid>(ctx, bc)));
//
//	ctx->PreProcess();
