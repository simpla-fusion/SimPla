...
// FIXME NEED parallel optimize
	...
	else
	{
		...
		static constexpr id_type sub_cell_flag[4][12] = { { 0x01, 0x02, 0x04,
		...
		static constexpr id_type sub_cell_shift[4][12] = { { 0, DI << 1, DJ
				<< 1, (DI | DJ) << 1,

		(DK << 1), (DK | DI) << 1, (DK | DJ) << 1, (DK | DI | DJ) << 1, }, {

		DI, DJ, DJ | (DI << 1), DI | (DJ << 1),

		DK, DK | (DI << 1), DK | (DJ << 1), DK | (DI << 1) | (DJ << 1),

		(DK << 1) | DI, (DK << 1) | DJ, (DK << 1) | DJ | (DI << 1), (DK << 1)
				| DI | (DJ << 1),

		}, { DI | DJ, DI | DK, DK | DJ, DI | DJ | (DK << 1), DI | DK
				| (DJ << 1), DK | DJ | (DI << 1) }, { DI | DJ | DK }
