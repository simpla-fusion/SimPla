...
public:
			...
			typename geometry_type::id_type s)
	{
		...
		typename geometry_type::id_type X = geometry_type::delta_index(
...
//
////	template<typename geometry_type,typename TM, size_t IL, typename TL> void calculate(
...
////			_Field<TL...> const & f, 		typename geometry_type::id_type   s)  = delete;
	...
	static inline traits::value_type_t<
			...
			typename geometry_type::id_type s)
	{
		typename geometry_type::id_type D = geometry_type::delta_index(
				geometry_type::dual(s));
		return
...
////! Form<IR> ^ Form<IR> => Form<IR+IL>
	...
	template<typename geometry_type, typename TL, typename TR>
	static inline traits::value_type_t<_Field<_impl::Wedge<EDGE, EDGE, TL, TR> > > calculate(
			geometry_type const & geo,
			_Field<_impl::Wedge<EDGE, EDGE, TL, TR>> const & expr,
			typename geometry_type::id_type s)
	{
		auto const & l = expr.lhs;
		auto const & r = expr.rhs;

		auto Y = geometry_type::delta_index(
				geometry_type::rotate(geometry_type::dual(s)));
	...
	template<typename geometry_type, typename TL, typename TR>
	static inline traits::value_type_t<_Field<_impl::Wedge<EDGE, FACE, TL, TR> > > calculate(
			geometry_type const & geo,
			_Field<_impl::Wedge<EDGE, FACE, TL, TR>> const & expr,
			typename geometry_type::id_type s)
	{
		auto const & l = expr.lhs;
		auto const & r = expr.rhs;
	...
	template<typename geometry_type, typename TL, typename TR>
	static inline traits::value_type_t<_Field<_impl::Wedge<FACE, EDGE, TL, TR> > > calculate(
			geometry_type const & geo,
			_Field<_impl::Wedge<FACE, EDGE, TL, TR>> const & expr,
			typename geometry_type::id_type s)
	{
		auto const & l = expr.lhs;
		auto const & r = expr.rhs;
...
////***************************************************************************************************
	...
	template<typename geometry_type, typename T>
	static inline traits::value_type_t<_Field<_impl::HodgeStar<VOLUME, T> > > calculate(
			geometry_type const & geo,
			_Field<_impl::HodgeStar<VOLUME, T>> const & expr,
			typename geometry_type::id_type s)
	{
...
//	template<typename geometry_type,typename TM, typename TL, typename TR> void calculate(
...
// Non-standard operation
	...
	template<typename geometry_type, typename T>
	traits::value_type_t<_Field<_impl::MapTo<EDGE, VERTEX, T>> > map_to(
			geometry_type const & geo,
			_Field<_impl::MapTo<EDGE, VERTEX, T>> const & expr,
			typename geometry_type::id_type s)
	{
		auto const & f = expr.lhs;
	...
	template<typename geometry_type, typename T>
	traits::value_type_t<_Field<_impl::MapTo<VERTEX, EDGE, T>> > map_to(
			geometry_type const & geo,
			_Field<_impl::MapTo<VERTEX, EDGE, T>> const & expr,
			typename geometry_type::id_type s)
	{
		auto const & f = expr.lhs;
		...
		return ((calculate(geo, f, s - D)[n] + calculate(geo, f, s + D)[n])
				* 0.5);
	}

	template<typename geometry_type, typename T>
	traits::value_type_t<_Field<_impl::MapTo<FACE, VERTEX, T>> > map_to(
			geometry_type const & geo,
			_Field<_impl::MapTo<FACE, VERTEX, T>> const & expr,
			typename geometry_type::id_type s)
	{
		auto const & f = expr.lhs;
		auto X = geo.DI(0, s);
	...
	template<typename geometry_type, typename T>
	traits::value_type_t<_Field<_impl::MapTo<VERTEX, FACE, T>> > map_to(
			geometry_type const & geo,
			_Field<_impl::MapTo<VERTEX, FACE, T>> const & expr,
			typename geometry_type::id_type s)
	{
		auto const & f = expr.lhs;
	...
	template<typename geometry_type, typename T>
	traits::value_type_t<_Field<_impl::MapTo<FACE, VOLUME, T>> > map_to(
			geometry_type const & geo,
			_Field<_impl::MapTo<FACE, VOLUME, T>> const & expr,
			typename geometry_type::id_type s)
	{
		auto const & f = expr.lhs;
	...
	template<typename geometry_type, typename T>
	traits::value_type_t<_Field<_impl::MapTo<VOLUME, FACE, T>> > map_to(
			geometry_type const & geo,
			_Field<_impl::MapTo<VOLUME, FACE, T>> const & expr,
			typename geometry_type::id_type s)
	{
		auto const & f = expr.lhs;
	...
	template<typename geometry_type, typename T>
	traits::value_type_t<_Field<_impl::MapTo<EDGE, VOLUME, T>> > map_to(
			geometry_type const & geo,
			_Field<_impl::MapTo<EDGE, VOLUME, T>> const & expr,
			typename geometry_type::id_type s)
	{
		auto const & f = expr.lhs;
	...
	template<typename geometry_type, typename T>
	traits::value_type_t<_Field<_impl::MapTo<VOLUME, EDGE, T>> > map_to(
			geometry_type const & geo,
			_Field<_impl::MapTo<VOLUME, EDGE, T>> const & expr,
			typename geometry_type::id_type s)
	{
		auto const & f = expr.lhs;
		auto n = geometry_type::component_number(geometry_type::dual(s));
