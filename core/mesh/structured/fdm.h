...
public:
	...
	static inline auto eval(mesh_type const & geo,
			_Field<Expression<TOP, T...> > const &f, id_type const &s)
			DECL_RET_TYPE(eval(geo,f,s,
							traits::iform_list_t<T...> () ))

	template<typename TOP, int I, typename ... T>
	static inline auto eval(mesh_type const & geo,
			_Field<Expression<TOP, std::integral_constant<int, I>, T...> > const &f,
			id_type const &s)
			DECL_RET_TYPE(eval(geo,f,s,
							traits::iform_list_t<T...> () ))
	...
	_Field<Expression<TOP, TL> > > > eval(
			mesh_type const & geo,
			_Field<Expression<TOP, TL>> const &f,
			id_type const & s,traits::iform_list_t<TL>)
	{
		traits::primary_type_t<traits::value_type_t<
		_Field<Expression<TOP, TL> > > > res;
		res = f.op_(eval(geo, f.lhs, s));
			...
			id_type s,integer_sequence<int,EDGE>)
	{
		...
		id_type X = mesh_type::delta_index(
				mesh_type::dual(s));
...
////			_Field<TL...> const & f, 		typename geometry_type::id_type   s)  = delete;
	...
	{
		id_type D = mesh_type::delta_index(
				mesh_type::dual(s));
		return
...
////! Form<IR> ^ Form<IR> => Form<IR+IL>
	...
	static inline traits::value_type_t<
	_Field<Expression<tags::Wedge, TL, TR> > > eval(
			mesh_type const & geo,
			_Field<Expression<tags::Wedge, TL, TR>> const & expr,
			id_type s ,integer_sequence<int,VERTEX, VERTEX>)
	{
		return (eval(geo,expr.lhs, s) * eval(geo,expr.rhs, s));
	}
	...
	_Field<Expression<tags::Wedge, TL, TR> > > eval(
			mesh_type& geo,
			_Field<Expression<tags::Wedge, TL, TR>> const & expr,
			id_type s,integer_sequence<int,VERTEX, EDGE>)
	{
		auto X = mesh_type::delta_index(s);

		return (eval(geo,expr.lhs, s - X) + eval(geo,expr.lhs, s + X)) * 0.5
		* eval(geo,expr.rhs, s);
	}
	...
	_Field<Expression<tags::Wedge, TL, TR> > > eval(
			mesh_type const & geo,
			_Field<Expression<tags::Wedge, TL, TR>> const & expr,
			id_type s,integer_sequence<int,VERTEX, FACE>)
	{
	...
	_Field<Expression<tags::Wedge, TL, TR> > > eval(
			mesh_type const & geo,
			_Field<Expression<tags::Wedge, TL, TR>> const & expr,
			id_type s,integer_sequence<int,VERTEX, VOLUME>)
	{
	...
	static inline traits::value_type_t<_Field<Expression<tags::Wedge, TL, TR> > > eval(
			mesh_type const & geo,
			_Field<Expression<tags::Wedge, TL, TR>> const & expr,
			id_type s,integer_sequence<int,EDGE, EDGE>)
	{
		auto const & l = expr.lhs;
		auto const & r = expr.rhs;
	...
	static inline traits::value_type_t<_Field<Expression<tags::Wedge, TL, TR> > > eval(
			mesh_type const & geo,
			_Field<Expression<tags::Wedge, TL, TR>> const & expr,
			id_type s,integer_sequence<int,EDGE, FACE>)
	{
		auto const & l = expr.lhs;
		auto const & r = expr.rhs;
		auto X = geo.DI(0, s);
	...
	static inline traits::value_type_t<_Field<Expression<tags::Wedge, TL, TR> > > eval(
			mesh_type const & geo,
			_Field<Expression<tags::Wedge, TL, TR> > const & expr,
			id_type s,integer_sequence<int,FACE, EDGE>)
	{
		auto const & l = expr.lhs;
		auto const & r = expr.rhs;
		auto X = geo.DI(0, s);
...
////***************************************************************************************************
	...
	static inline traits::value_type_t<_Field<Expression<tags::HodgeStar, T> > > eval(
			mesh_type const & geo,
			_Field<Expression<tags::HodgeStar , T> > const & expr,
			id_type s,integer_sequence< int,VOLUME >)
	{
