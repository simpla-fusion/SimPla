...
struct Metric<CS, tags::constant_space>
{
	typedef CS cs_type;
...
//	}
	 ...
	 *              x                       y
	 **/
	point_type point(id_type const &s) const { return std::move(map(mids::coordinates(s))); }
	...
	auto pull_back(point_type const &x, TFun const &fun) const DECL_RET_TYPE((fun(map(x))))

	template<typename TFun>
	auto push_forward(point_type const &y, TFun const &fun) const DECL_RET_TYPE((fun(inv_map(y))))
	...
	{
		return std::move(mids::coordinates_global_to_local(coordinates_to_topology(x), n_id));
	}
};
