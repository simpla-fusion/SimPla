...
public:
	...
	typedef Point3 coordinate_tuple;
...
private:
	...
	coordinate_tuple m_rzmin_;
	coordinate_tuple m_rzmax_;
...
//	coordinate_tuple rzmin_;
//	coordinate_tuple rzmax_;
...
//	inter_type qpsi_;//!< q values on uniform flux grid from axis to boundary
	...
	Polygon<coordinate_tuple> m_rzbbb_; //!< R,Z of boundary points in meter
	Polygon<coordinate_tuple> m_rzlim_; //!< R,Z of surrounding limiter contour in meter
...
//	void Write(std::string const &fname);
	...
	inline Real profile(std::string const & name,
			coordinate_tuple const & x) const
	{
	...
	inline std::vector<coordinate_tuple> const & boundary() const
	{
		return m_rzbbb_;
	}
	inline std::vector<coordinate_tuple> const & limiter() const
	{
		return m_rzlim_;
	}
	...
	inline Real psi(coordinate_tuple const&x) const
	{
	...
	inline auto B(coordinate_tuple const&x) const
	...
	inline auto JT(coordinate_tuple const&x) const
	...
	coordinate_tuple map_cylindrical_to_flux(
			coordinate_tuple const & psi_theta_phi, size_t VecZAxis = 2) const;

	coordinate_tuple map_flux_from_cylindrical(coordinate_tuple const & x,
	 ...
	 */
	bool flux_surface(Real psi_j, size_t M, coordinate_tuple*res,
	 ...
	 *
	 */
	bool map_to_flux_coordiantes(std::vector<coordinate_tuple> const&surface,
...
//	{
//
//		f->pull_back(*this, [this](coordinate_tuple const & x)
//		{	return this->B(x);});
//
//	}
//	else
...
//	{
//
//		f->pull_back(*this, [this,name](coordinate_tuple const & x)
