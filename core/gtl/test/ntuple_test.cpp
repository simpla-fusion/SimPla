...
protected:
	...
	virtual void SetUp()
	{
		...
		seq_for_each(dimensions(),
				...
				{
					traits::try_access_r(aA,idx) = idx[0] * 2;
					traits::try_access_r(aB,idx) = 5 - idx[0];
					traits::try_access_r(aC,idx) = idx[0] * 5 + 1;
					traits::try_access_r(aD,idx) = 0;
					traits::try_access_r(vA,idx) = traits::try_access_r(aA,idx);
					traits::try_access_r(vB,idx) = traits::try_access_r(aB,idx);
					traits::try_access_r(vC,idx) = traits::try_access_r(aC,idx);
					traits::try_access_r(vD,idx) = 0;

					traits::try_access_r(res,idx) = -(traits::try_access_r(aA,idx) + a) /
					(traits::try_access_r(aB,idx) * b - c) - traits::try_access_r(aC,idx);
...
TYPED_TEST(TestNtuple, swap){
{
	...
	seq_for_each(typename TestFixture::dimensions(),
			...
			{
				EXPECT_DOUBLE_EQ(0, std::abs(traits::try_access_r(TestFixture::aA,idx)- traits::try_access_r(TestFixture:: vB,idx)));
				EXPECT_DOUBLE_EQ(0, std::abs(traits::try_access_r(TestFixture::aB,idx)- traits::try_access_r(TestFixture:: vA,idx)));
			});
...
TYPED_TEST(TestNtuple, assign_Scalar){
{
	...
	seq_for_each(typename TestFixture::dimensions(),
			[&](size_t const idx[TestFixture::dimensions::size()])
			{
				EXPECT_DOUBLE_EQ(0, std::abs(TestFixture::a- traits::try_access_r(TestFixture:: vA,idx)));
			}
	);
...
TYPED_TEST(TestNtuple, assign_Array){
{
	...
	seq_for_each(typename TestFixture::dimensions(),
			[&](size_t const idx[TestFixture::dimensions::size()])
			{
				EXPECT_DOUBLE_EQ(0, std::abs(traits::try_access_r(TestFixture::aA,idx)- traits::try_access_r(TestFixture:: vA,idx)));
			}
	);
...
TYPED_TEST(TestNtuple, self_assign){
{
	...
	seq_for_each(typename TestFixture::dimensions(),
			[&](size_t const idx[TestFixture::dimensions::size()])
			{
				EXPECT_DOUBLE_EQ(0,std::abs( traits::try_access_r(TestFixture::vB,idx)
								- (traits::try_access_r(TestFixture::aB,idx)+
										traits::try_access_r(TestFixture::aA,idx))));
...
TYPED_TEST(TestNtuple, arithmetic){
{
	...
	seq_for_each(typename TestFixture::dimensions(),
			[&](size_t const idx[TestFixture::dimensions::size()])
			{
				auto &ta=traits::try_access_r(TestFixture::vA,idx);
				auto &tb=traits::try_access_r(TestFixture::vB,idx);
				auto &tc=traits::try_access_r(TestFixture::vC,idx);
				auto &td=traits::try_access_r(TestFixture::vD,idx);

				EXPECT_DOUBLE_EQ(0, std::abs(EQUATION(ta,tb,tc ) - td));
			}
	);
...
//	vC=cross(vA,vB);
//	vD-=vC;
//	EXPECT_DOUBLE_EQ(0,std::abs(vD[0])+std::abs(vD[1])+std::abs(vD[2]) );
//}}
//
...
//			[&](size_t const idx[TestFixture::dimensions::size()])
//			{
//				expect+=traits::try_access_r(TestFixture::vA,idx);
//			}
//	);
//	auto value=seq_reduce(typename TestFixture::dimensions(),_impl::plus(), TestFixture::vA);
//
//	EXPECT_DOUBLE_EQ(0,std::abs(expect -value));
//
//}
//}
//
...
//			[&](size_t const idx[TestFixture::dimensions::size()])
//			{
//				res += traits::try_access_r(TestFixture::aA,idx) * traits::try_access_r(TestFixture::aB,idx);
//			}
//	);
//
//	EXPECT_DOUBLE_EQ(0,std::abs(res- inner_product( TestFixture::vA, TestFixture::vB)));
//}}
//
