...
public:
	...
	struct Slot
	{
		...
		// member function
		template<typename T>
		Slot(std::shared_ptr<T> &o_ptr, result_type (T::*ps)(Args ...)) :
				m_tracker_(std::dynamic_pointer_cast<AbstractTracker>(std::make_shared<Tracker<T> >(o_ptr)))
		{
			m_slot_ = [=](Args &&...args)
			{
			    return (o_ptr.get()->*ps)(std::forward<Args>(args)...);
			};
		}
	...
	result_type call(std::false_type, Args &&... args) const
	{
		result_type res;
		...
		for (auto const &item:m_slots_)
		{
			if (item)
			{
				res = item(std::forward<Args>(args) ...);
			}
		}
