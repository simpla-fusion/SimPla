...
{
	typedef TCoord coordinate_type;

	std::vector<coordinate_type> points;

	std::vector<coordinate_type> intersect_points;
	...
	while (first != points.end())
	{
		...
		std::sort(ib, intersect_points.end(),
				[&](coordinate_type const & xa, coordinate_type const & xb)
				{
...
void filter_domain_by_config(TDict const & dict, TDomain * domain)
{
	...
	else
	{
		if (dict.is_function())
		{
			domain->filter_by_coordinates([&](coordinate_type const & x )
			{
