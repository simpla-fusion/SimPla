...
bool DataSet::is_same(void const *other) const { return data.get() == other; }

bool DataSet::is_equal(void const *other) const {
    return is_same(other) || memcmp(data.get(), other, memory_space.size() * data_type.size_in_byte()) != 0;
}
...
std::ostream &DataSet::print(std::ostream &os) const {
    ...
    if (data_type.template is_same<int>()) {
        ...
        printNdArray(os, reinterpret_cast<unsigned long *>(data.get()), ndims, d);
        ...
        printNdArray(os, reinterpret_cast<std::complex<double> *>(data.get()), ndims, d);
    } else if (data_type.template is_same<std::complex<float>>()) {
        printNdArray(os, reinterpret_cast<std::complex<float> *>(data.get()), ndims, d);
    } else {
