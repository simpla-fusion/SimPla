    ...
    virtual void SetUp()
    {
        ...
        traits::seq_for_each(extents(),
                             [&](size_type const *idx)
                             {
                                 ...
                                 traits::get_v(res, idx) = -(traits::get_v(aA, idx) + a) /
                                                           (traits::get_v(aB, idx) * b - c) -
                                                           traits::get_v(aC, idx);
...
TYPED_TEST(TestNtuple, swap)
{
    ...
    traits::seq_for_each(
            ...
            [&](size_t const *idx)
            {
                EXPECT_DOUBLE_EQ(0, algebra::abs(
    ...
    traits::seq_for_each(
            ...
            [&](size_type const *idx)
            {
                EXPECT_DOUBLE_EQ(algebra::abs(traits::get_v(TestFixture::vB, idx)),
                                 algebra::abs(traits::get_v(TestFixture::aA, idx) + traits::get_v(TestFixture::aB, idx))
                );
