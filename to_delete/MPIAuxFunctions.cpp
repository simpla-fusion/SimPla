...
//    MPI_Barrier(comm);
//    MPI_Reduce(const_cast<void *>(send_data), (recv_data), count, m_type.type(), get_MPI_Op(op_c), 0, comm);
//    MPI_Barrier(comm);
//}
//
...
//	{
//
//		MPI_Isend( reinterpret_cast<byte_type*>(const_cast<void* >(m_send_links_))+std::Serialize<3>(item) ,
//				std::get<4>(item), MPI_BYTE, std::get<0>(item), std::Serialize<1>(item),
...
//		MPI_Status status;
//
//		MPI_Probe(std::get<0>(item), std::Serialize<2>(item), GLOBAL_COMM.comm(), &status);
//
//		// When probe returns, the status object find the size and other
//		// attributes of the incoming message. Serialize the size of the message
//		int tmp = 0;
//		MPI_Get_count(&status, MPI_BYTE, &tmp);
//
//		if (tmp == MPI_UNDEFINED)
//		{
//			THROW_RUNTIME_ERROR("UpdatePatch Ghosts Particle fail");
//		}
//		else
//		{
...
//	{
//
//		MPI_Irecv(m_recv_links_.get() + pos, mem_size[i], MPI_BYTE, std::get<0>(info[i]), std::Serialize<2>(info[i]),
//				GLOBAL_COMM.comm(), &requests[req_count] );
//
//		pos+= mem_size[i];
//		++req_count;
