/*Copyright (C) 2007-2014 YU Zhi. All rights reserved.
 *
 * cor_rect_mesh_rz.h
 *
 * Created on: 20014-2-19
 * Author: salmon
 */
#ifndef UNIFORM_RECT_RZ_H_
#define UNIFORM_RECT_RZ_H_
...
struct CoRectMeshRZ
{
	typedef CoRectMeshRZ this_type;
	...
	nTuple<NUM_OF_DIMS, Real> dS_[2] = { 0, 0, 0, 0, 0, 0 };
...
public:
	...
	inline size_t GetArrayIndex(IDXS ... s) const
	{
		return GetComponentIndex(IFORM,s...);
	}
...
public:
	...
	inline index_type GetAdjacentCells(Int2Type<IN>, Int2Type<OUT>, index_type s, index_type *v) const
	{
...
//		CHECK(ix)<<" "<<jx<<" "<<kx<<" "<<std::hex<<d;
	...
	size_t GetArrayIndex(TI const & ...s)const
	{
		return static_cast<size_t>(GetIndex(s...));
	}
...
template<typename ISTREAM> inline void CoRectMeshRZ::Load(ISTREAM const &cfg)
{
...
template<typename ...Args>
void CoRectMeshRZ::ParallelTraversal(Args const &...args) const
{
...
template<typename ...Args>
void CoRectMeshRZ::SerialTraversal(Args const &...args) const
{
