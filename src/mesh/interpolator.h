...
public:
	...
	static inline typename TF::value_type Gather_(TF const &f, coordinates_type r, compact_index_type shift)
	{
		...
		f[((s + X) + Y) + Z] * mesh.geometry_type::InvVolume(((s + X) + Y) + Z) * (r[0]) * (r[1]) * (r[2])
		        + f[((s + X) + Y) - Z] * mesh.geometry_type::InvVolume(((s + X) + Y) - Z) * (r[0]) * (r[1])
		                * (1.0 - r[2])
		        + f[((s + X) - Y) + Z] * mesh.geometry_type::InvVolume(((s + X) - Y) + Z) * (r[0]) * (1.0 - r[1])
		                * (r[2])
		        + f[((s + X) - Y) - Z] * mesh.geometry_type::InvVolume(((s + X) - Y) - Z) * (r[0]) * (1.0 - r[1])
		                * (1.0 - r[2])
		        + f[((s - X) + Y) + Z] * mesh.geometry_type::InvVolume(((s - X) + Y) + Z) * (1.0 - r[0]) * (r[1])
		                * (r[2])
		        + f[((s - X) + Y) - Z] * mesh.geometry_type::InvVolume(((s - X) + Y) - Z) * (1.0 - r[0]) * (r[1])
		                * (1.0 - r[2])
		        + f[((s - X) - Y) + Z] * mesh.geometry_type::InvVolume(((s - X) - Y) + Z) * (1.0 - r[0]) * (1.0 - r[1])
		                * (r[2])
		        + f[((s - X) - Y) - Z] * mesh.geometry_type::InvVolume(((s - X) - Y) - Z) * (1.0 - r[0]) * (1.0 - r[1])
		                * (1.0 - r[2]);
	}
	...
	static inline void Scatter_(coordinates_type r, typename TF::value_type const & v, compact_index_type shift, TF *f)
	{
		...
		f->get(((s + X) + Y) + Z) += v * mesh.geometry_type::Volume(((s + X) + Y) + Z) * (r[0]) * (r[1]) * (r[2]);
		f->get(((s + X) + Y) - Z) += v * mesh.geometry_type::Volume(((s + X) + Y) - Z) * (r[0]) * (r[1]) * (1.0 - r[2]);
		f->get(((s + X) - Y) + Z) += v * mesh.geometry_type::Volume(((s + X) - Y) + Z) * (r[0]) * (1.0 - r[1]) * (r[2]);
		f->get(((s + X) - Y) - Z) += v * mesh.geometry_type::Volume(((s + X) - Y) - Z) * (r[0]) * (1.0 - r[1])
		        * (1.0 - r[2]);
		f->get(((s - X) + Y) + Z) += v * mesh.geometry_type::Volume(((s - X) + Y) + Z) * (1.0 - r[0]) * (r[1]) * (r[2]);
		f->get(((s - X) + Y) - Z) += v * mesh.geometry_type::Volume(((s - X) + Y) - Z) * (1.0 - r[0]) * (r[1])
		        * (1.0 - r[2]);
		f->get(((s - X) - Y) + Z) += v * mesh.geometry_type::Volume(((s - X) - Y) + Z) * (1.0 - r[0]) * (1.0 - r[1])
		        * (r[2]);
		f->get(((s - X) - Y) - Z) += v * mesh.geometry_type::Volume(((s - X) - Y) - Z) * (1.0 - r[0]) * (1.0 - r[1])
		        * (1.0 - r[2]);
	}
	...
	static inline void Scatter(coordinates_type x, typename Field<mesh_type, VERTEX, TExpr>::field_value_type const & v,
	        Field<mesh_type, VERTEX, TExpr> *f)
	{
		Scatter_(x, v, 0UL, f);
	}
	...
	static inline void Scatter(coordinates_type x, typename Field<mesh_type, VOLUME, TExpr>::field_value_type const & v,
	        Field<mesh_type, VOLUME, TExpr> *f)
	{
