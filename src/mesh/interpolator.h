...
public:
	...
	static inline typename TF::value_type Gather_(TF const &f, coordinates_type r, compact_iterator shift)
	{
		...
		auto X = shift.DeltaIndex(0);
		auto Y = shift.DeltaIndex(1);
		auto Z = shift.DeltaIndex(2);

		auto s = mesh.topology_type::CoordinatesGlobalToLocal(&r, shift) + shift.DeltaIndex();

		return

		f[((s + X) + Y) + Z] * mesh.geometry_type::InvVolume(((s + X) + Y) + Z) * (r[0]) * (r[1]) * (r[2])

		+ f[((s + X) + Y) - Z] * mesh.geometry_type::InvVolume(((s + X) + Y) - Z) * (r[0]) * (r[1]) * (1.0 - r[2])

		+ f[((s + X) - Y) + Z] * mesh.geometry_type::InvVolume(((s + X) - Y) + Z) * (r[0]) * (1.0 - r[1]) * (r[2])

		+ f[((s + X) - Y) - Z] * mesh.geometry_type::InvVolume(((s + X) - Y) - Z) * (r[0]) * (1.0 - r[1]) * (1.0 - r[2])

		+ f[((s - X) + Y) + Z] * mesh.geometry_type::InvVolume(((s - X) + Y) + Z) * (1.0 - r[0]) * (r[1]) * (r[2])

		+ f[((s - X) + Y) - Z] * mesh.geometry_type::InvVolume(((s - X) + Y) - Z) * (1.0 - r[0]) * (r[1]) * (1.0 - r[2])

		+ f[((s - X) - Y) + Z] * mesh.geometry_type::InvVolume(((s - X) - Y) + Z) * (1.0 - r[0]) * (1.0 - r[1]) * (r[2])
