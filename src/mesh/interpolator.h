...
public:
	...
	static inline typename TF::value_type Gather_(TF const &f,
			coordinates_type r, compact_index_type shift)
	{
		mesh_type const & mesh = f.mesh;
		r = mesh.geometry_type::CoordinatesGlobalToLocal(r);
		...
		auto s = mesh.topology_type::CoordinatesGlobalToLocal(&r, shift)
				+ (topology_type::_DA >> (topology_type::H(shift) + 1));

		return

		f[((s + X) + Y) + Z] * mesh.geometry_type::InvVolume(((s + X) + Y) + Z)
				* (r[0]) * (r[1]) * (r[2])

				+ f[((s + X) + Y) - Z]
						* mesh.geometry_type::InvVolume(((s + X) + Y) - Z)
						* (r[0]) * (r[1]) * (1.0 - r[2])
				+ f[((s + X) - Y) + Z]
						* mesh.geometry_type::InvVolume(((s + X) - Y) + Z)
						* (r[0]) * (1.0 - r[1]) * (r[2])
	...
	static inline typename Field<mesh_type, VERTEX, TExpr>::field_value_type Gather(
			Field<mesh_type, VERTEX, TExpr> const &f, coordinates_type x,
			unsigned long h = 0)
	{
	...
	static inline typename Field<mesh_type, EDGE, TExpr>::field_value_type Gather(
			Field<mesh_type, EDGE, TExpr> const &f, coordinates_type x,
			unsigned long h = 0)
	{
	...
	static inline typename Field<mesh_type, FACE, TExpr>::field_value_type Gather(
			Field<mesh_type, FACE, TExpr> const &f, coordinates_type x,
			unsigned long h = 0)
	{
		return typename Field<mesh_type, EDGE, TExpr>::field_value_type(
				{
				...
				Gather_(f, x,
						topology_type::ShiftH(
								(topology_type::_DJ | topology_type::_DK) >> 1,
								h)),

				Gather_(f, x,
						topology_type::ShiftH(
								(topology_type::_DK | topology_type::_DI) >> 1,
								h)),

				Gather_(f, x,
						topology_type::ShiftH(
								(topology_type::_DI | topology_type::_DJ) >> 1,
								h)) });

	}
	template<typename TExpr>
	static inline typename Field<mesh_type, VOLUME, TExpr>::field_value_type Gather(
			Field<mesh_type, VOLUME, TExpr> const &f, coordinates_type x,
			unsigned long h = 0)
	{
	...
	template<typename TF>
	static inline void Scatter_(coordinates_type r,
			typename TF::value_type const & v, compact_index_type shift, TF *f)
	{
		...
		f->get(((s + X) + Y) + Z) += v
				* mesh.geometry_type::Volume(((s + X) + Y) + Z) * (r[0]) * (r[1])
				* (r[2]);
		f->get(((s + X) + Y) - Z) += v
				* mesh.geometry_type::Volume(((s + X) + Y) - Z) * (r[0]) * (r[1])
				* (1.0 - r[2]);
		f->get(((s + X) - Y) + Z) += v
				* mesh.geometry_type::Volume(((s + X) - Y) + Z) * (r[0])
				* (1.0 - r[1]) * (r[2]);
		f->get(((s + X) - Y) - Z) += v
				* mesh.geometry_type::Volume(((s + X) - Y) - Z) * (r[0])
				* (1.0 - r[1]) * (1.0 - r[2]);
		f->get(((s - X) + Y) + Z) += v
				* mesh.geometry_type::Volume(((s - X) + Y) + Z) * (1.0 - r[0])
				* (r[1]) * (r[2]);
		f->get(((s - X) + Y) - Z) += v
				* mesh.geometry_type::Volume(((s - X) + Y) - Z) * (1.0 - r[0])
				* (r[1]) * (1.0 - r[2]);
	...
	template<typename TExpr>
	static inline void Scatter(coordinates_type x,
			typename Field<mesh_type, VERTEX, TExpr>::field_value_type const & v,
	...
	template<typename TExpr>
	static inline void Scatter(coordinates_type x,
			typename Field<mesh_type, EDGE, TExpr>::field_value_type const & v,
	...
	template<typename TExpr>
	static inline void Scatter(coordinates_type x,
			typename Field<mesh_type, FACE, TExpr>::field_value_type const & v,
			Field<mesh_type, FACE, TExpr> *f, unsigned long h = 0)
	{

		Scatter_(x, v[0],
				topology_type::ShiftH(
						(topology_type::_DJ | topology_type::_DK) >> 1, h), f);

		Scatter_(x, v[1],
				topology_type::ShiftH(
						(topology_type::_DK | topology_type::_DI) >> 1, h), f);

		Scatter_(x, v[2],
				topology_type::ShiftH(
						(topology_type::_DI | topology_type::_DJ) >> 1, h), f);
	}

	template<typename TExpr>
	static inline void Scatter(coordinates_type x,
			typename Field<mesh_type, VOLUME, TExpr>::field_value_type const & v,
