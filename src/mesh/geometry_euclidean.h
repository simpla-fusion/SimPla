...
struct EuclideanGeometry
{
	...
	void Load(TDict const & dict, Others const &...)
	{
		if (dict["Min"] && dict["Max"])
		{
			...
			SetExtent(dict["Min"].template as<nTuple<3, Real>>(),
	...
	template<typename T>
	inline void SetExtent(nTuple<NDIMS, T> const & pmin,
	...
	void Update()
	{
		...
		volume_[1] /* 001 */=
				(dims[0] > 1) ?
						(xmax_[0] - xmin_[0]) / static_cast<Real>(dims[0]) : 1;
		volume_[2] /* 010 */=
				(dims[1] > 1) ?
						(xmax_[1] - xmin_[1]) / static_cast<Real>(dims[1]) : 1;
		volume_[4] /* 100 */=
				(dims[2] > 1) ?
						(xmax_[2] - xmin_[2]) / static_cast<Real>(dims[2]) : 1;
