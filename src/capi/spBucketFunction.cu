...
spPageGroupDestroy (spPageGroup **pg)
{
  if (pg != 0 && *pg != 0)
	{
	  spPageGroup *t = (*pg);
 ...
 */
MC_HOST_DEVICE_PREFIX size_type
spPageGroupSize (spPageGroup const *pg)
{
...
MC_HOST_DEVICE_PREFIX void
spPagePoolCreate (spPagePool **res, size_type size_in_byte)
{
  *res = 0x0;
...
MC_HOST_DEVICE_PREFIX void
spPagePoolDestroy (spPagePool **pool)
{
...
MC_HOST_DEVICE_PREFIX void
spPagePoolReleaseEnpty (spPagePool *pool)
{
...
MC_HOST_DEVICE_PREFIX size_type
spEntityFill (spPage *p, size_type num, const byte_type *src)
{
  while (num > 0 && p != 0x0)
	{
	  size_type n =
...
MC_HOST_DEVICE_PREFIX spEntity *
...
MC_HOST_DEVICE_PREFIX void
spEntityRemove (spPage *p, bucket_entity_flag_t flag)
{
  p->flag &= (~flag);
}
...
MC_HOST_DEVICE_PREFIX void
spEntityCopyIf (spPage *src, spPage **dest, id_type tag, spPagePool *pool)
{
...
MC_HOST_DEVICE_PREFIX int
spBucketEnternalSort (spPage **src, spPage **dest)
{
  return 0;
}
