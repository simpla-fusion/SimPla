...
struct _Field<Expression<T...>> : public Expression<T...>
{
	...
	typedef typename field_traits<this_type>::value_type value_type;
	typedef typename field_traits<this_type>::index_type index_type;
	typedef typename field_traits<this_type>::domain_type domain_type;

	inline auto operator[](index_type const &s) const
	DECL_RET_TYPE (( field_traits<this_type>::calculate(*this)))

	auto domain() const
	DECL_RET_TYPE (( field_traits<this_type>::get_domain(*this)))
...
template<typename T> using Field=_Field<std::vector<T> >;
