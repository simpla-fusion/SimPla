...
//		}
	...
	{
		return iterator_<const container_type>(data_,
				mesh.GetRange(IForm).begin());
	}

	const_iterator end() const
	{
		return iterator_<const container_type>(data_,
				mesh.GetRange(IForm).end());
	}
	...
	void Assign(TD const &f)
	{
		...
		[&](index_type s)
		{
			this->get(s) = f(this->mesh.GetCoordinates(s));
		}
