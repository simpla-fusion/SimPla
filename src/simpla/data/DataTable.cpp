...
struct DataTable::pimpl_s
{
    ...
    std::shared_ptr<DataEntity> &emplace(DataTable *t, std::string const &url, char split_char = '.');

    DataEntity const *search(DataTable const *, std::string const &url, char split_char = '.');
};

std::shared_ptr<DataEntity> &
DataTable::pimpl_s::emplace(DataTable *t, std::string const &url, char split_char)
{
    ...
    while (start_pos < end_pos)
    {
        ...
        {
            auto res = t->m_pimpl_->m_table_.emplace(url.substr(start_pos), std::make_shared<LightData>());
...
DataEntity const *
DataTable::pimpl_s::search(DataTable const *t, std::string const &url, char split_char)
{
    size_type start_pos = 0;
    size_type end_pos = url.size();
    while (start_pos < end_pos)
...
bool
DataTable::empty() const { return (m_pimpl_ != nullptr) && m_pimpl_->m_table_.empty(); };

bool
DataTable::has(std::string const &url) const { return find(url) != nullptr; };


DataTable *
DataTable::create_table(std::string const &url) { return &m_pimpl_->emplace(this, url + ".")->as_table(); }

std::shared_ptr<DataEntity> &DataTable::set(std::string const &url, std::shared_ptr<DataEntity> const &v)
{
...
std::shared_ptr<DataEntity> &DataTable::get(std::string const &url) { return m_pimpl_->emplace(this, url); }
...
void
DataTable::foreach(std::function<void(std::string const &, DataEntity const &)> const &fun) const
{
    for (auto &item: m_pimpl_->m_table_) { fun(item.first, *std::dynamic_pointer_cast<const DataEntity>(item.second)); }
}

void
DataTable::foreach(std::function<void(std::string const &, DataEntity &)> const &fun)
{
