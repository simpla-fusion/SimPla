...
#include <simpla/algebra/ndArray.h>
...
class HeavyDataAdapter<ndArray<V, N>> {
    typedef ndArray<V, N> array_type;
    SP_OBJECT_HEAD(array_type, HeavyData);

   public:
    typedef V value_type;

    static constexpr std::true_type is_owner;
    ...
    explicit HeavyDataAdapter(std::shared_ptr<value_type> const &p, Args &&... args)
        ...
        initialize(std::forward<Args>(args)...);
        //        Intialize(ndims, lo, hi, order, i_lo, i_hi);
    };

    HeavyDataAdapter(this_type const &other) = delete;

    virtual ~HeavyDataAdapter(){};
   ...
   public:
    ...
    index_type const *count() const { return m_count_; }

    size_type size() const { return m_size_; }

    virtual bool is_valid() const {
        return (m_data_ != nullptr) && (m_count_[0] >= 1) && (m_count_[1] >= 1) &&
               (m_count_[2] >= 1) && (m_count_[3] >= 1) && (m_count_[4] >= 1);
    };

    virtual std::ostream &print(std::ostream &os, int indent = 1) const {
        os << "-- dims=[" << m_count_[0] << "," << m_count_[1] << "," << m_count_[2] << ","
           << m_count_[3] << ","
           << "]" << std::endl;
    ...
    value_type &get(Args &&... args) {
        return m_data_[hash(std::forward<Args>(args)...) % m_size_];
    }
    ...
    inline constexpr size_type hash(index_type x0, index_type x1, index_type x2,
                                    index_type x3) const {
    ...
    inline constexpr size_type hash2(index_type const *start, size_type const *count) const {
        return 0;
    }

    template <typename... Args>
    inline constexpr size_type hash2(index_type const *start, size_type const *count,
                                     size_type const *stride, index_type first,
    ...
    template <typename... Args>
    inline constexpr size_type hash(index_type x0, index_type x1, index_type x2, index_type x3,
                                    index_type s4, Args const &... args) const {
            ...
            for (index_type i = ib; i < ie; ++i)
                for (index_type j = jb; j < je; ++j)
                    for (index_type k = kb; k < ke; ++k)
                        for (index_type l = lb; l < le; ++l) { fun(get(i, j, k, l), i, j, k, l); }
...
#pragma omp parallel for
                for (index_type i = ib; i < ie; ++i)
                    for (index_type j = jb; j < je; ++j)
                        for (index_type k = kb; k < ke; ++k) { fun(get(i, j, k, l), i, j, k, l); }
            }
        }
    };
...
//                << "size ={" << m_count_[0] << "," << m_count_[1] << "," << m_count_[2] << "},"
