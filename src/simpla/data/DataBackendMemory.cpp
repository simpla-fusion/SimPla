...
void DataBackendMemory::Set(std::string const& uri, std::shared_ptr<DataEntity> const& v, bool overwrite) {
    ...
    //        src_table.Foreach(
    //            [&](std::string const& k, std::shared_ptr<DataEntity> const& v) { dest_table.Unpack(k, v, overwrite);
    //            });
    ...
    //        for (size_type i = 0, ie = src_array.size(); i < ie; ++i) { dest_array->Add(src_array.Pack(i)); }
