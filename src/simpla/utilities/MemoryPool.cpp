...
MemoryPool::MemoryPool() : pimpl_(new pimpl_s) { pimpl_->pool_depth_ = 0; }
MemoryPool::~MemoryPool() { clear(); }

//!  unused memory will be freed when total memory size >= pool size
void MemoryPool::max_size(size_t s) { pimpl_->max_pool_depth_ = s; }

/**
 *  return the total size of memory in pool
 * @return
 */
double MemoryPool::size() const { return static_cast<double>(pimpl_->pool_depth_); }

void MemoryPool::clear() { pimpl_->clear(); }
...
void MemoryPool::push(void *p, size_t s) { pimpl_->push(p, s); }
