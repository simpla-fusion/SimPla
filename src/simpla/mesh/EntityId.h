   ...
   public:
    ...
    size_t size() const {
        return static_cast<size_t>(((m_iform_ == VERTEX || m_iform_ == VOLUME) ? 1 : 3) *
                                   (m_max_[0] - m_min_[0]) * (m_max_[1] - m_min_[1]) *
                                   (m_max_[2] - m_min_[2]));
    }
    ...
    void foreach (
        TFun const& body,
        ENABLE_IF((simpla::concept::is_callable<TFun(mesh::MeshEntityId const&)>::value))) const {
...
#pragma omp parallel for
    ...
    void foreach (TFun const& body,
                  ENABLE_IF((simpla::concept::is_callable<TFun(index_type, index_type, index_type,
