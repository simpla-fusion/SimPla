...
struct MeshIdHashCompare {
    static constexpr inline bool equal(const MeshEntityId& l, const MeshEntityId& r) {
        return l.v == r.v;
    }
...
constexpr inline MeshEntityId operator-(MeshEntityId const& first, MeshEntityId const& second) {
    return MeshEntityId{first.w, static_cast<u_int16_t>(first.z - second.z),
...
struct MeshEntityIdCoder_ {
    ...
    static constexpr MeshEntityId _DI{1, 0, 0, 0};
    static constexpr MeshEntityId _DJ{0, 1, 0, 0};
    ...
    static MeshEntityId tag(MeshEntityId s, int64_t tag) {
        s.v = (s.v & (~_DA.v)) | tag;
        return s;
    }
    ...
    static constexpr int iform(MeshEntityId s) { return m_id_to_iform_[node_id(s)]; }

    static constexpr MeshEntityId pack(index_type i0, index_type i1, index_type i2,
                                       index_type w = 0) {
        return MeshEntityId{static_cast<u_int16_t>(w), static_cast<u_int16_t>(i2),
                            static_cast<u_int16_t>(i1), static_cast<u_int16_t>(i0)};
    }
    ...
    static constexpr MeshEntityId pack_index(index_type i, index_type j, index_type k,
                                             index_type n_id = 0, index_type w = 0) {
        return pack((i + ZERO) << 1, (j + ZERO) << 1, (k + ZERO) << 1, w) | m_id_to_shift_[n_id];
    }

    template <int IFORM>
    static constexpr MeshEntityId pack_index4(index_type i, index_type j, index_type k,
                                              index_type n_id = 0, index_type w = 0) {
    ...
    static index_tuple unpack_index(MeshEntityId const& s) {
        return index_tuple{static_cast<index_type>(s.x >> 1) - ZERO,
    ...
    static nTuple<index_type, 4> unpack_index4(MeshEntityId const& s, index_type dof = 1) {
        return nTuple<index_type, 4>{
            static_cast<index_type>(s.x >> 1) - ZERO, static_cast<index_type>(s.y >> 1) - ZERO,
            static_cast<index_type>(s.z >> 1) - ZERO,
    ...
    static std::tuple<MeshEntityId, point_type> point_global_to_local(point_type const& x,
                                                                      int n_id = 0) {
    ...
    static point_type point_local_to_global(MeshEntityId s, point_type const& x) {
        return point(s) + x * _R * 2;
    }
    ...
    static constexpr MeshEntityId DI(int n) { return m_num_to_id_[n]; }
    ...
    static constexpr MeshEntityId rotate(MeshEntityId const& s) {
        return MeshEntityId{static_cast<u_int16_t>(s.w),
    ...
    static constexpr int sub_index(MeshEntityId const& s) { return m_id_to_index_[node_id(s)]; }
        ...
        m_adjacent_cell_matrix_[4 /* to iform*/][NUM_OF_NODE_ID /* node id*/]
                               [MAX_NUM_OF_ADJACENT_CELL /*id shift*/] = {
                                   ...
                                   {// To EDGE
                                    ...
                                    {_DA - _DJ, _DA + _DI, _DA + _DJ, _DA - _DI},  //      /* 011*/
    ...
    static int get_adjacent_entities(int IFORM, int nodeid, MeshEntityId s,
                                     MeshEntityId* res = nullptr) {
        if (res != nullptr) {
    ...
    template <int IFORM>
    static constexpr size_type max_hash(MeshEntityId b, MeshEntityId e) {
...
template <int L>
constexpr int MeshEntityIdCoder_<L>::ndims;

template <int L>
constexpr int MeshEntityIdCoder_<L>::MESH_RESOLUTION;

template <int L>
constexpr Real MeshEntityIdCoder_<L>::_R;

template <int L>
constexpr MeshEntityId MeshEntityIdCoder_<L>::_DK;

template <int L>
constexpr MeshEntityId MeshEntityIdCoder_<L>::_DJ;

template <int L>
constexpr MeshEntityId MeshEntityIdCoder_<L>::_DI;

template <int L>
constexpr MeshEntityId MeshEntityIdCoder_<L>::_DA;

template <int L>
constexpr int MeshEntityIdCoder_<L>::m_id_to_index_[];

template <int L>
constexpr int MeshEntityIdCoder_<L>::m_id_to_iform_[];

template <int L>
constexpr int MeshEntityIdCoder_<L>::m_id_to_num_of_ele_in_cell_[];

template <int L>
constexpr int MeshEntityIdCoder_<L>::m_adjacent_cell_num_[4][8];

template <int L>
constexpr int MeshEntityIdCoder_<L>::m_iform_to_num_of_ele_in_cell_[];

template <int L>
constexpr MeshEntityId MeshEntityIdCoder_<L>::m_num_to_id_[];

template <int L>
constexpr MeshEntityId MeshEntityIdCoder_<L>::m_id_to_shift_[];

template <int L>
constexpr int MeshEntityIdCoder_<L>::m_id_to_sub_index_[];

template <int L>
constexpr int MeshEntityIdCoder_<L>::m_sub_index_to_id_[4][3];
...
template <int L>
constexpr Real MeshEntityIdCoder_<L>::m_id_to_coordinates_shift_[8][3];

typedef MeshEntityIdCoder_<> MeshEntityIdCoder;

}  // namespace  get_mesh
template <>
struct ContinueRange<mesh::MeshEntityId> : public RangeBase<mesh::MeshEntityId> {
   private:
    static constexpr int ndims = 3;
   ...
   public:
    ...
    ContinueRange(index_tuple const& b, index_tuple const& e) : ContinueRange(&b[0], &(e[0])) {}

    ContinueRange(std::tuple<index_tuple, index_tuple> const& b)
        : ContinueRange(std::get<0>(b), std::get<1>(b)) {}

    ContinueRange(this_type const& r)
        : m_min_(r.m_min_), m_max_(r.m_max_), m_grain_size_(r.m_grain_size_) {}
    ...
    std::shared_ptr<base_type> split(concept::tags::split const& proportion) {
        ...
        m_max_[n] = m_min_[n] +
                    L * proportion.left() / ((proportion.left() + proportion.right() > 0)
    ...
    bool empty() const {
        return m_min_[0] == m_max_[0] || m_min_[1] == m_max_[1] || m_min_[2] == m_max_[2];
    }

    size_t size() const {
        return static_cast<size_t>((m_max_[0] - m_min_[0]) * (m_max_[1] - m_min_[1]) *
                                   (m_max_[2] - m_min_[2]));
    }
...
#pragma omp parallel for
        for (index_type i = ib; i < ie; ++i) {
            for (index_type j = r.m_min_[1], je = r.m_max_[1]; j < je; ++j)
                for (index_type k = r.m_min_[2], ke = r.m_max_[2]; k < ke; ++k) { body(i, j, k); }
        }
    }
