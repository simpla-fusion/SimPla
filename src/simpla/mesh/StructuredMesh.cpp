...
void StructuredMesh::InitializeRange(std::shared_ptr<geometry::GeoObject> const& g, Range<EntityId> body[4],
    ...
    auto idx_box = GetBlock()->GetIndexBox();
    {
        ...
        ie += 1;
        body[VERTEX].append(std::make_shared<ContinueRange<EntityId>>(ib, ie, 0));
    }

    {
        ...
        for (int i = 0; i < 3; ++i) {
            ...
            ie[(i + 1) % 3] += 1;
            ie[(i + 2) % 3] += 1;
            body[EDGE].append(std::make_shared<ContinueRange<EntityId>>(ib, ie, 0b1 << i));
        }
    }

    {
        ...
        for (int i = 0; i < 3; ++i) {
            std::tie(ib, ie) = idx_box;
            ie[i] += 1;
            body[FACE].append(std::make_shared<ContinueRange<EntityId>>(ib, ie, (~(0b1 << i)) & 0b111));
        }
    }
    {
        index_tuple ib, ie;
        std::tie(ib, ie) = idx_box;
        body[VOLUME].append(std::make_shared<ContinueRange<EntityId>>(ib, ie, 0b111));
    }
    //        } break;
    //        case 0:
    //        default: {
