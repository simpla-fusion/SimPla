...
#pragma omp parallel for
        for (int i = 0; i < box.m_inner_count_[0]; ++i)
            for (int j = 0; j < box.m_inner_count_[1]; ++j)
                for (int k = 0; k < box.m_inner_count_[2]; ++k)
                {
                    fun(hash(i + box.m_inner_start_[0], j + box.m_inner_start_[1], k + box.m_inner_start_[2]));
                }
    }
