...
{   \
   ...
   static inline unsigned int from(lua_State* L,  unsigned int  idx, _TYPE_ * v )                       \
    {if (!check(L,idx)){return 0;}else{*v = static_cast<_TYPE_>(_FROM_FUN_(L, idx)); return 1;}}    \
   static inline unsigned int  to(lua_State* L, _TYPE_ const & v) { _TO_FUN_(L, v); return 1;} \
};
...
private:
                                     ...
                                     std::integral_constant<unsigned int, N>)
    {
        ...
        auto num = _impl::pop_from_lua(L, -1,
                                       &std::get<N - 1>(*v)); // C++ tuple index start from 0
        lua_pop(L, 1);
