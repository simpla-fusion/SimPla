...
void XDMFStream::open_grid(const std::string &g_name, int TAG)
{
    ...
    switch (TAG)
    {
        ...
        case UNIFORM://
            ...
            m_file_stream_
                    << std::endl
                    << std::setw(level * 2) << "" << "<Grid Name=\"" << g_name << "\" GridType=\"Uniform\">"
                    << std::endl;
            break;
    ...
    if (ndims == 3)
    {
        m_file_stream_ << ""
                       ...
                       << "  <DataItem Name=\"Origin\" Dimensions=\"3\" NumberType=\"Float\" "
                               " Precision=\"4\" Format=\"XML\">" << xmin[0] << " " << xmin[1] << " " << xmin[2]
                       << "</DataItem>\n"
                       << std::setw(level * 2 + 2) << ""
                       << "  <DataItem Name=\"Spacing\" Dimensions=\"3\" NumberType=\"Float\""
                               " Precision=\"4\" Format=\"XML\">" << dx[0] << " " << dx[1] << " " << dx[2]
                       << "  </DataItem >\n"
...
void XDMFStream::set_topology_geometry(std::string const &name, data::DataSet const &ds)
{
    ...
    if (ndims == 2)
    {
        m_file_stream_ << ""
                       ...
                       << std::setw(level * 2 + 2) << "<geometry Name=  \"" << name << "\" GeometryType=\"XY\">"
                       << std::endl;
