...
SP_REGISTER_CREATOR(Entry, DataEntryMemory);
DataEntryMemory::DataEntryMemory(Entry::eNodeType e_type) : base_type(e_type){};
...
std::shared_ptr<Entry> DataEntryMemory::Copy() const {
    return std::shared_ptr<Entry>(new DataEntryMemory(*this));
}

size_type DataEntryMemory::size() const { return m_table_.size(); }

size_type DataEntryMemory::Set(index_type s, const std::shared_ptr<Entry> &v) { return Set(std::to_string(s), v); }
size_type DataEntryMemory::Add(index_type s, const std::shared_ptr<Entry> &v) { return Add(std::to_string(s), v); }
size_type DataEntryMemory::Add(const std::shared_ptr<Entry> &v) { return Set(std::to_string(size()), v); };
size_type DataEntryMemory::Delete(index_type s) { return Delete(std::to_string(s)); }
std::shared_ptr<const Entry> DataEntryMemory::Get(index_type s) const { return Get(std::to_string(s)); }
std::shared_ptr<Entry> DataEntryMemory::Get(index_type s) { return Get(std::to_string(s)); }

size_type DataEntryMemory::Set(std::string const &uri, const std::shared_ptr<Entry> &v) {
...
size_type DataEntryMemory::Add(std::string const &uri, const std::shared_ptr<Entry> &v) {
    if (uri.empty() || v == nullptr) { return 0; }
    if (uri[0] == SP_URL_SPLIT_CHAR) { return Root()->Set(uri.substr(1), v); }
...
std::shared_ptr<const Entry> DataEntryMemory::Get(std::string const &uri) const {
...
std::shared_ptr<Entry> DataEntryMemory::Get(std::string const &uri) {
    if (uri.empty()) { return nullptr; }
    if (uri[0] == SP_URL_SPLIT_CHAR) { return Root()->Get(uri.substr(1)); }
...
void DataEntryMemory::Foreach(
    std::function<void(std::string const &, std::shared_ptr<const Entry> const &)> const &f) const {
    for (auto const &item : m_table_) { f(item.first, item.second); }
}
void DataEntryMemory::Foreach(std::function<void(std::string const &, std::shared_ptr<Entry> const &)> const &f) {
    for (auto const &item : m_table_) { f(item.first, item.second); }
};

std::shared_ptr<Entry> DataEntryMemory::CreateNode(eNodeType e_type) const {
