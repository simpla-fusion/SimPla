...
struct DataEntryXDMF : public DataEntryMemory {
    ...
    void WriteDataItem(std::string const &url, std::string const &key, const index_box_type &idx_box,
                       std::shared_ptr<const data::Entry> const &data, int indent);
    void WriteAttribute(std::string const &url, std::string const &key, const index_box_type &idx_box,
                        std::shared_ptr<const data::Entry> const &attr, int indent);
...
SP_REGISTER_CREATOR(Entry, DataEntryXDMF);

DataEntryXDMF::DataEntryXDMF(Entry::eNodeType etype)
...
void XDMFGeometryCurvilinear(DataEntryXDMF *self, std::string const &prefix, const index_box_type &idx_box,
                             std::shared_ptr<const Entry> const &chart,
                             std::shared_ptr<const data::Entry> const &coord, int indent) {
    index_tuple lo, hi;
    std::tie(lo, hi) = idx_box;
    hi += 1;
...
void XDMFGeometryRegular(DataEntryXDMF *self, const index_box_type &idx_box,
                         std::shared_ptr<const Entry> const &chart, int indent = 0) {
    index_tuple lo, hi;
    std::tie(lo, hi) = idx_box;
    hi += 1;
...
int DataEntryXDMF::Flush() {
    ...
    if (auto atlas = this->Get("Atlas")) {
        ...
        if (auto patches = atlas->Get("Patches")) {
            patches->Foreach([&](std::string const &k, std::shared_ptr<const data::Entry> const &patch) {
                ...
                *m_os_ << std::setw(indent) << " "
                ...
                attrs->Foreach([&](std::string const &s, std::shared_ptr<const data::Entry> const &d) {
