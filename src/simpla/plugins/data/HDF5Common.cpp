...
size_type HDF5Set(hid_t g_id, std::string const& key, std::shared_ptr<const Entry> node) {
...
size_type HDF5Add(hid_t g_id, std::string const& key, std::shared_ptr<const Entry> node) {
    if (node == nullptr) { return 0; }
    size_type count = 0;
    switch (node->type()) {
        case Entry::DN_ARRAY:
