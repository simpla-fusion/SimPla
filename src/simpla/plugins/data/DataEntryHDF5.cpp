...
struct DataEntryHDF5 : public Entry {
    SP_DATA_ENTITY_HEAD(Entry, DataEntryHDF5, h5)
   ...
   public:
    ...
    size_type Set(std::string const& uri, std::shared_ptr<const Entry> const& v) override;
    size_type Set(index_type s, std::shared_ptr<const Entry> const& v) override;

    size_type Add(std::string const& uri, std::shared_ptr<const Entry> const& v) override;
    size_type Add(index_type s, std::shared_ptr<const Entry> const& v) override;
...
SP_REGISTER_CREATOR(Entry, DataEntryHDF5);
...
DataEntryHDF5::DataEntryHDF5(Entry::eNodeType e_type)
...
std::shared_ptr<Entry> DataEntryHDF5::CreateNode(eNodeType e_type) const {
...
std::shared_ptr<Entry> DataEntryHDF5::Get(std::string const& uri) {
            ...
            if (H5Lexists(*p->m_group_, sub_key.c_str(), H5P_DEFAULT) &&
                ...
                switch (o_info.type) {
                    ...
                    case H5O_TYPE_DATASET: {
                        ...
                        obj = Entry::New(HDF5GetEntity(d_id, false));
                ...
                if (a_id > -1) {
                    obj = Entry::New(HDF5GetEntity(a_id, true));
...
std::shared_ptr<const Entry> DataEntryHDF5::Get(std::string const& uri) const {
    if (uri.empty()) { return nullptr; }
    if (uri[0] == SP_URL_SPLIT_CHAR) { return Root()->Get(uri.substr(1)); }
...
size_type DataEntryHDF5::Set(std::string const& uri, const std::shared_ptr<const Entry>& v) {
...
size_type DataEntryHDF5::Add(std::string const& uri, const std::shared_ptr<const Entry>& v) {
    if (uri.empty() || v == nullptr) { return 0; }
    if (uri[0] == SP_URL_SPLIT_CHAR) { return Root()->Set(uri.substr(1), v); }
...
void DataEntryHDF5::Foreach(
    std::function<void(std::string const&, std::shared_ptr<const Entry> const&)> const& fun) const {
...
//}
size_type DataEntryHDF5::Set(index_type s, const std::shared_ptr<const Entry>& v) {
    return Set(std::to_string(s), v);
}
size_type DataEntryHDF5::Add(index_type s, const std::shared_ptr<const Entry>& v) {
    return Add(std::to_string(s), v);
}
size_type DataEntryHDF5::Delete(index_type s) { return Delete(std::to_string(s)); }
std::shared_ptr<const Entry> DataEntryHDF5::Get(index_type s) const { return Get(std::to_string(s)); }
std::shared_ptr<Entry> DataEntryHDF5::Get(index_type s) { return Get(std::to_string(s)); }
