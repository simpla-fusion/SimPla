...
struct calculator<TM> {
    ...
    //    static auto getValue(mesh_type const& m, TFun const& fun, int n,
    //                         ENABLE_IF(simpla::concept::is_callable<TFun(EntityId)>::value)) {
    //        return sample(m, n, fun);
    //    }
    //
    //    template <typename TFun>
    //    static auto getValue(mesh_type const& m, TFun const& fun, int n,
    ...
    static auto eval(mesh_type const& m, TExpr const& expr, int n, IdxShift S,
    ...
    static auto eval(mesh_type const& m, TExpr const& expr, int n, IdxShift S,
        ...
        return ((getV(m, expr, (n + 1) % 3, S + Y) -  //
                 getV(m, expr, (n + 1) % 3, S)) +     //
                (getV(m, expr, (n + 2) % 3, S + Z) -  //
                 getV(m, expr, (n + 2) % 3, S))) *    //
               (-IVolume(m, n, S));
    }
    ...
    static auto eval(mesh_type const& m, TExpr const& expr, int n, IdxShift S,
        ...
        return ((getDualV(m, expr, 0, S) - getDualV(m, expr, 0, S - X)) +  //
                (getDualV(m, expr, 1, S) - getDualV(m, expr, 1, S - Y)) +  //
                (getDualV(m, expr, 2, S) - getDualV(m, expr, 2, S - Z))) *
               (-IDVolume(m, n, S));
        ;
    }
    ...
    static auto eval(mesh_type const& m, TExpr const& expr, int n, IdxShift S,
        ...
        return (getV(m, l, n, S) - getV(m, l, n, S - D)) * (-IDVolume(m, n, S));
    }
    //
    //    template<typename T>
    ...
    static auto eval(mesh_type const& m, TExpr const& expr, int n, IdxShift S, expression_tag<tags::_map_to<IL>, IR>) {
    ...
    template <typename TExpr, int IL, int IR>
    static auto eval(mesh_type const& m, TExpr const& expr, int n, IdxShift S, expression_tag<tags::_wedge, IL, IR>) {
    ...
    template <typename TExpr, int I>
    static auto eval(mesh_type const& m, TExpr const& expr, int n, IdxShift S, expression_tag<tags::_wedge, I, I>)  //
    {
        IdxShift X{0, 0, 0};
        IdxShift Y{0, 0, 0};
        IdxShift Z{0, 0, 0};
        ...
        Y[(n + 1) % 3] = 1;
        Z[(n + 2) % 3] = 1;
        return getValue(m, std::get<0>(expr.m_args_), n, S + Z) * getValue(m, std::get<1>(expr.m_args_), n, S + Y) -
               getValue(m, std::get<0>(expr.m_args_), n, S + Z) * getValue(m, std::get<1>(expr.m_args_), n, S + Y);
    }

    template <typename TExpr, int I>
    static auto eval(mesh_type const& m, TExpr const& expr, int n, IdxShift S, expression_tag<tags::_dot, I, I>)  //
    {
        IdxShift X{1, 0, 0};
        IdxShift Y{0, 1, 0};
        IdxShift Z{0, 0, 1};

        return getValue(m, std::get<0>(expr.m_args_), n, S + X) * getValue(m, std::get<1>(expr.m_args_), n, S + X) +
               getValue(m, std::get<0>(expr.m_args_), n, S + Y) * getValue(m, std::get<1>(expr.m_args_), n, S + Y) +
               getValue(m, std::get<0>(expr.m_args_), n, S + Z) * getValue(m, std::get<1>(expr.m_args_), n, S + Z);
    }
    template <typename TExpr, int I, int K>
    static auto eval(mesh_type const& m, TExpr const& expr, int n, IdxShift S, expression_tag<tags::_cross, I, K>)  //
    {
        IdxShift X{0, 0, 0};
        IdxShift Y{0, 0, 0};
        IdxShift Z{0, 0, 0};
        ...
        Y[(n + 1) % 3] = 1;
        Z[(n + 2) % 3] = 1;
        return getValue(m, std::get<0>(expr.m_args_), n, S + Y) * getValue(m, std::get<1>(expr.m_args_), n, S + Z) -
               getValue(m, std::get<0>(expr.m_args_), n, S + Z) * getValue(m, std::get<1>(expr.m_args_), n, S + Y);
    }
    ...
    template <typename TExpr, int... I>
    static auto _invoke_helper(mesh_type const& m, TExpr const& expr, int n, IdxShift S, int_sequence<I...>) {
        return expr.m_op_(getValue(m, std::get<I>(expr.m_args_), n, S)...);
    }

    template <typename TExpr, typename TOP, int... I>
    static auto eval(mesh_type const& m, TExpr const& expr, int n, IdxShift S, expression_tag<TOP, I...>) {
    ...
    static auto getValue(mesh_type const& m, Expression<TOP, T...> const& expr, int n, IdxShift S) {
        return eval(m, expr, n, S, expression_tag<TOP, traits::iform<T>::value...>());
    }
    template <typename V, int I, int D>
    static auto getValue(mesh_type const& m, Field<mesh_type, V, I, D> const& f, int n, IdxShift S) {
        return f[n / D][n % D](S);
    };
    template <typename V, int I, int D>
    static auto getValue(mesh_type const& m, Field<mesh_type, V, I, D>& f, int n, IdxShift S) {
        return f[n / D][n % D](S);
    };

    template <typename V, int I, int D>
    static V const& getValue(mesh_type const& m, Field<mesh_type, V, I, D> const& f, EntityId s) {
        return f[EntityIdCoder::m_id_to_sub_index_[s.w % 0b111]][s.w >> 3](s.x, s.y, s.z);
    };
    template <typename V, int I, int D>
    static V& getValue(mesh_type const& m, Field<mesh_type, V, I, D>& f, EntityId s) {
        return f[EntityIdCoder::m_id_to_sub_index_[s.w % 0b111]][s.w >> 3](s.x, s.y, s.z);
    };
    template <typename T>
    static T const& getValue(mesh_type const& mesh_type, T const& v, int n, IdxShift S,
...
//        return v[EntityIdCoder::sub_index(s)];
//    }
//
//    template <typename TV>
//    static auto sample_(mesh_type const& m, EntityId s, nTuple<TV, 3> const& v) {
...
//    }
//
//    template<typename mesh_type,int IFORM,  typename TV>
...
//    static void foreach_(mesh_type const& m, TField& self, Range<EntityId> const& r, TOP const& op, Args&&...
//    args) {
