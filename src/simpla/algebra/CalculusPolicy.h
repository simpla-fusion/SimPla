...
struct calculator {
    ...
    static V const& getValue(Field<TM, V, I, D> const& f, mesh_type const& m, EntityId s) {
        return f[M::m_id_to_sub_index_[s.w & 0b111] * D + (s.w >> 3)](s.x, s.y, s.z);
    };
    template <typename V, int I, int D>
    ...
    static auto _invoke_helper(Expression<TOP, T...> const& expr, mesh_type const& m, int n, IdxShift S,
                               int_sequence<I...>) {
        return expr.m_op_(getValue(std::get<I>(expr.m_args_), m, n, S)...);
    }
    ...
    template <typename TMesh, typename V, int I, int D>
    static auto getValue(Field<TMesh, V, I, D> const& f, mesh_type const& m, int tag, IdxShift S = IdxShift{0, 0, 0}) {
        return f[M::m_id_to_sub_index_[(tag & 0b111)] * D + (tag >> 3) % D](S);
    };
    template <typename TMesh, typename V, int I, int D>
    static auto getValue(Field<TMesh, V, I, D>& f, mesh_type const& m, int tag, IdxShift S = IdxShift{0, 0, 0}) {
        return f[M::m_id_to_sub_index_[tag & 0b111] * D + (tag >> 3) % D](S);
    };
    ...
    static T const& getValue(T const* v, mesh_type const& m, int tag, IdxShift S = IdxShift{0, 0, 0},
                             ENABLE_IF((std::is_arithmetic<T>::value))) {
        return v[(((tag & 0b111) == 0) || ((tag & 0b111) == 7)) ? (tag >> 3) : M::m_id_to_sub_index_[tag & 0b111]];
    }
    ...
    //
    //    template <typename TExpr>
    //    static auto getDualV(mesh_type const& m, TExpr const& expr, ENABLE_IF((traits::iform<TExpr>::value ==
    //    VERTEX))) {
    ...
    template <typename TExpr>
    static auto hodge(TExpr const& expr, mesh_type const& m, int tag, IdxShift S,
                      ENABLE_IF((traits::iform<TExpr>::value == VERTEX))) {
        return getValue(expr, m, tag, S) * getValue(m.m_vertex_hodge_, m, tag, S);
    }
    template <typename TExpr>
    static auto hodge(TExpr const& expr, mesh_type const& m, int tag, IdxShift S,
                      ENABLE_IF((traits::iform<TExpr>::value == EDGE))) {
        return getValue(expr, m, tag, S) * getValue(m.m_edge_hodge_, m, tag, S);
    }
    template <typename TExpr>
    static auto hodge(TExpr const& expr, mesh_type const& m, int tag, IdxShift S,
                      ENABLE_IF((traits::iform<TExpr>::value == FACE))) {
        return getValue(expr, m, tag, S) * getValue(m.m_face_hodge_, m, tag, S);
    }
    template <typename TExpr>
    static auto hodge(TExpr const& expr, mesh_type const& m, int tag, IdxShift S,
                      ENABLE_IF((traits::iform<TExpr>::value == VOLUME))) {
        return getValue(expr, m, tag, S) * getValue(m.m_volume_hodge_, m, tag, S);
    }

    template <typename TExpr>
    static auto inv_hodge(TExpr const& expr, mesh_type const& m, int tag, IdxShift S,
                          ENABLE_IF((traits::iform<TExpr>::value == VERTEX))) {
        return getValue(expr, m, tag, S) * getValue(m.m_vertex_inv_hodge_, m, tag, S);
    }
    template <typename TExpr>
    static auto inv_hodge(TExpr const& expr, mesh_type const& m, int tag, IdxShift S,
                          ENABLE_IF((traits::iform<TExpr>::value == EDGE))) {
        return getValue(expr, m, tag, S) * getValue(m.m_edge_inv_hodge_, m, tag, S);
    }
    template <typename TExpr>
    static auto inv_hodge(TExpr const& expr, mesh_type const& m, int tag, IdxShift S,
                          ENABLE_IF((traits::iform<TExpr>::value == FACE))) {
        return getValue(expr, m, tag, S) * getValue(m.m_face_inv_hodge_, m, tag, S);
    }
    template <typename TExpr>
    static auto inv_hodge(TExpr const& expr, mesh_type const& m, int tag, IdxShift S,
                          ENABLE_IF((traits::iform<TExpr>::value == VOLUME))) {
        return getValue(expr, m, tag, S) * getValue(m.m_volume_inv_hodge_, m, tag, S);
    }
    ...
    template <typename TExpr>
    static auto eval(Expression<tags::_exterior_derivative, TExpr> const& expr, mesh_type const& m, int s, IdxShift S,
                     int_sequence<VERTEX>) {
        auto const& l = std::get<0>(expr.m_args_);
        IdxShift D{0, 0, 0};
        D[M::m_id_to_sub_index_[s % 0b111]] = 1;
        return (getValue(l, m, s & (~0b111), S + D) - getValue(l, m, s & (~0b111), S));
    }
    ...
    static auto eval(Expression<tags::_exterior_derivative, TExpr> const& expr, mesh_type const& m, int s, IdxShift S,
        ...
        int n = M::m_id_to_sub_index_[s];

        Y[(n + 1) % 3] = 1;
        Z[(n + 2) % 3] = 1;

        return ((getValue(l, m, (s & (~0b111)) | M::m_sub_index_to_id_[EDGE][(n + 1) % 3], S + Z) -
                 getValue(l, m, (s & (~0b111)) | M::m_sub_index_to_id_[EDGE][(n + 1) % 3], S)) -
                (getValue(l, m, (s & (~0b111)) | M::m_sub_index_to_id_[EDGE][(n + 2) % 3], S + Y) -
                 getValue(l, m, (s & (~0b111)) | M::m_sub_index_to_id_[EDGE][(n + 2) % 3], S)));
    }
    ...
    static auto eval(Expression<tags::_exterior_derivative, TExpr> const& expr, mesh_type const& m, int s, IdxShift S,
        ...
        return ((getValue(l, m, (s & (~0b111)) | M::m_sub_index_to_id_[FACE][0], S + X) -
                 getValue(l, m, (s & (~0b111)) | M::m_sub_index_to_id_[FACE][0], S)) +  //
                (getValue(l, m, (s & (~0b111)) | M::m_sub_index_to_id_[FACE][1], S + Y) -
                 getValue(l, m, (s & (~0b111)) | M::m_sub_index_to_id_[FACE][1], S)) +  //
                (getValue(l, m, (s & (~0b111)) | M::m_sub_index_to_id_[FACE][2], S + Z) -
                 getValue(l, m, (s & (~0b111)) | M::m_sub_index_to_id_[FACE][2], S))  //
                );
    }
    ...
    static auto eval(Expression<tags::_codifferential_derivative, TExpr> const& expr, mesh_type const& m, int tag,
        ...
        int n = M::m_id_to_sub_index_[tag % 0b111];

        Y[(n + 1) % 3] = 1;
        Z[(n + 2) % 3] = 1;

        return (hodge(l, m, (tag & (~0b111)) | M::m_sub_index_to_id_[FACE][(n + 1) % 3], S) -
                hodge(l, m, (tag & (~0b111)) | M::m_sub_index_to_id_[FACE][(n + 1) % 3], S - Z)) -
               (hodge(l, m, (tag & (~0b111)) | M::m_sub_index_to_id_[FACE][(n + 2) % 3], S) -
                hodge(l, m, (tag & (~0b111)) | M::m_sub_index_to_id_[FACE][(n + 2) % 3], S - Y)) *
                   getValue(-m.m_edge_inv_hodge_, m, tag, S);
    }
    ...
    static auto eval(Expression<tags::_codifferential_derivative, TExpr> const& expr, mesh_type const& m, int tag,
        ...
        return ((hodge(l, m, (tag & (~0b111)) | M::m_sub_index_to_id_[EDGE][0], S) -
                 hodge(l, m, (tag & (~0b111)) | M::m_sub_index_to_id_[EDGE][0], S - X)) +
                (hodge(l, m, (tag & (~0b111)) | M::m_sub_index_to_id_[EDGE][1], S) -
                 hodge(l, m, (tag & (~0b111)) | M::m_sub_index_to_id_[EDGE][1], S - Y)) +
                (hodge(l, m, (tag & (~0b111)) | M::m_sub_index_to_id_[EDGE][2], S) -
                 hodge(l, m, (tag & (~0b111)) | M::m_sub_index_to_id_[EDGE][2], S - Z))) *
               getValue(-m.m_vertex_inv_hodge_, m, tag, S);
    ...
    static auto eval(Expression<tags::_codifferential_derivative, TExpr> const& expr, mesh_type const& m, int tag,
        ...
        auto const& l = std::get<0>(expr.m_args_);
        IdxShift D{0, 0, 0};
        D[M::m_id_to_sub_index_[tag & 0b111]] = 1;

        return (hodge(l, m, tag | 0b111, S) - hodge(l, m, tag | 0b111, S - D)) *
               getValue(-m.m_face_inv_hodge_, m, tag, S);
    }
    ...
    //        return (                                                //
    //                   getValue(l, m, id, S) +                      //
    //                   getValue(l, m, id, S + IdxShift{0, 0, 1}) +  //
    //                   getValue(l, m, id, S + IdxShift{0, 1, 0}) +  //
    //                   getValue(l, m, id, S + IdxShift{0, 1, 1}) +  //
    //                   getValue(l, m, id, S + IdxShift{1, 0, 0}) +  //
    //                   getValue(l, m, id, S + IdxShift{1, 0, 1}) +  //
    //                   getValue(l, m, id, S + IdxShift{1, 1, 0}) +  //
    //                   getValue(l, m, id, S + IdxShift{1, 1, 1})    //
    //                   ) *
    //               getValue(m.m_volume_inv_hodge_, m, tag, S) * 0.125;
    //    };
    ////***************************************************************************************************
    //! p_curl<1>
    ...
    static auto _map_to(TExpr const& expr, mesh_type const& m, int tag, IdxShift S, std::index_sequence<EDGE, VERTEX>) {
        IdxShift D{0, 0, 0};
        int n = (tag >> 3) % 3;
        int id = M::m_sub_index_to_id_[EDGE][n];
        D[n] = 1;
    ...
    static auto _map_to(TExpr const& expr, mesh_type const& m, int tag, IdxShift S, std::index_sequence<FACE, VERTEX>) {
        int n = (tag >> 3) % 3;
        int id = M::m_sub_index_to_id_[FACE][n];
    ...
    static auto _map_to(TExpr const& expr, mesh_type const& m, int tag, IdxShift S, std::index_sequence<VOLUME, FACE>) {
        int n = M::m_id_to_sub_index_[tag];
        int id = (n << 3) | 0b111;
        IdxShift D{0, 0, 0};
        D[n] = 1;
        return (getValue(expr, m, id, S - D) + getValue(expr, m, id, S)) * 0.5;
    ...
    static auto eval(Expression<simpla::tags::_map_to<IDest>, TExpr> const& expr, mesh_type const& m, int s, IdxShift S,
                     int_sequence<ISrc>) {
        return _map_to(std::get<0>(expr.m_args_), m, s, S, std::index_sequence<ISrc, IDest>());
    }
    //***************************************************************************************************
    //
    ...
    template <typename... TExpr>
    static auto eval(Expression<simpla::tags::_wedge, TExpr...> const& expr, mesh_type const& m, int s, IdxShift S,
        ...
        return (getValue(l, m, (s & (~0b111)) | M::m_sub_index_to_id_[EDGE][(n + 2) % 3], S) +       //
                getValue(l, m, (s & (~0b111)) | M::m_sub_index_to_id_[EDGE][(n + 2) % 3], S + Y)) *  //
               (getValue(l, m, (s & (~0b111)) | M::m_sub_index_to_id_[EDGE][(n + 1) % 3], S) +       //
                getValue(l, m, (s & (~0b111)) | M::m_sub_index_to_id_[EDGE][(n + 1) % 3], S + Z)) *  //
               0.25;
    }
   ...
   public:
    ...
    static auto sample_(mesh_type const& m, EntityId s, nTuple<TV, N> const& v) {
        return v[((s.w & 0b111) == 0 || (s.w & 0b111) == 7) ? (s.w >> 3) % N : M::m_id_to_sub_index_[s.w & 0b11]];
    }
    ...
    //        return getValue( expr,m, M::Pack<IFORM>(i, j, k, n, d));
    //    }
    //    template <typename TField, typename TOP, typename... Args>
    //    void foreach_(mesh_type const& m, TField& self, Range<EntityId> const& r, TOP const& op, Args&&... args)
    //    const
    //    {
