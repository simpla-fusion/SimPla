...
struct nTuple_<TV, N0, NOthers...> {
    ...
    sub_type const&
    at(size_type s) const {
        return data_[s];
    }

    value_type&
    at(size_type const* s) {
        return calculator::get_value(data_, s);
    }

    value_type const&
    at(size_type const* s) const {
        return calculator::get_value(data_, s);
    }
    ...
    nTuple_(simpla::traits::nested_initializer_list_t<value_type,
                                                      sizeof...(NOthers) + 1>
                l) {
        simpla::traits::assign_nested_initializer_list<N0, NOthers...>::apply(
            data_, l);
    }
...
struct calculator<declare::nTuple_<V, J...>> {
   ...
   public:
    ...
    static decltype(auto)
    get_value(T* v, TI const* s) {
        return get_value(v[*s], s + 1);
    };

    template <typename T, size_type... N, typename Idx>
    static decltype(auto)
    get_value(declare::nTuple_<T, N...>& v, Idx const* idx) {
        return get_value(v.data_, idx);
    };

    template <typename T, size_type... N, typename Idx>
    static decltype(auto)
    get_value(declare::nTuple_<T, N...> const& v, Idx const* idx) {
        return get_value(v.data_, idx);
    };
    ...
    _invoke_helper(declare::Expression<TOP, Others...> const& expr,
        ...
        AUTO_RETURN((expr.m_op_(get_value(std::get<index>(expr.m_args_),
                                          std::forward<Idx>(s)...)...)))
            ...
            static decltype(auto)
                get_value(declare::Expression<TOP, Others...> const& expr,
                          Idx&&... s)  //
        AUTO_RETURN((_invoke_helper(expr, index_sequence_for<Others...>(),
                                    std::forward<Idx>(s)...)))
