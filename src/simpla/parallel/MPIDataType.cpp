...
//#include "../utilities/PrettyStream.h"

namespace simpla {

MPIDataType::MPIDataType() : m_type_(MPI_DATATYPE_NULL) {}
...
MPIDataType::~MPIDataType() {
    if (is_commited_) { MPI_CALL(MPI_Type_free(&m_type_)); }
}
...
MPIDataType MPIDataType::create(data::DataType const &data_type,  //
    ...
    if (data_type.is_compound()) {
        ...
        for (auto const &item : data_type.members()) {
            ...
            for (int i = 0; i < sub_datatype.rank(); ++i) { block_length *= sub_datatype.extent(i); }
    ...
    if (data_type.is_array() || (ndims > 0 && p_dims != nullptr)) {
        ...
        MPI_Type_create_subarray(ndims + data_type.rank(), &l_dims[0], &l_count[0], &l_offset[0],
                                 (c_order_array ? MPI_ORDER_C : MPI_ORDER_FORTRAN), ele_type, &res_type);

        if (!is_predefined) { MPI_Type_free(&ele_type); }
        is_predefined = false;
    }

    if (!is_predefined) { MPI_CALL(MPI_Type_commit(&res_type)); }
...
MPIDataType MPIDataType::create(data::DataType const &data_type, data::DataSpace const &d_space, bool c_order_array) {
    ...
    return create(data_type, ndims, &dimensions[0], &start[0], &stride[0], &count[0], &block[0], c_order_array);
}
