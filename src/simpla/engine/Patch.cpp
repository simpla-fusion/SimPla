...
struct Patch::pimpl_s {
    ...
    std::map<id_type, std::shared_ptr<data::DataEntity>> m_data_;
};
...
int Patch::SetDataBlock(id_type const &id, std::shared_ptr<data::DataEntity> const &d) {
    auto res = m_pimpl_->m_data_.emplace(id, d);
    if (res.first->second == nullptr) { res.first->second = d; }
    return res.first->second != nullptr ? 1 : 0;
}
std::shared_ptr<data::DataEntity> Patch::GetDataBlock(id_type const &id) const {
    auto it = m_pimpl_->m_data_.find(id);
    return it == m_pimpl_->m_data_.end() ? nullptr : it->second;
}
std::map<id_type, std::shared_ptr<data::DataEntity>> &Patch::GetAllDataBlock() const { return m_pimpl_->m_data_; };
}  // namespace engine {
}  // namespace simpla {