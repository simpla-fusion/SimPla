...
#define SP_CALL(_CMD_)                                                                                      \
    {                                                                                                       \
        if (SP_SUCCESS != print_error((_CMD_), __FILE__, __LINE__, __PRETTY_FUNCTION__, __STRING(_CMD_))) { \
            return SP_FAILED;                                                                               \
        }                                                                                                   \
    }
...
#define SP_DEVICE_CALL(_CMD_)                              \
    {                                                      \
        auto error_code = (_CMD_);                         \
        if (error_code != SP_SUCCESS) {                    \
            RUNTIME_ERROR << __STRING(_CMD_) << std::endl; \
            return SP_FAILED;                              \
        }                                                  \
    }
...
#define SP_CALL_DEVICE_KERNEL(_FUN_, _DIMS_, _N_THREADS_, ...)              \
    ...
    {                                                                       \
        ...
        if (SP_SUCCESS != error_code) {                                     \
            RUNTIME_ERROR << "CUDA ERROR code:" << error_code << std::endl; \
            return SP_FAILED;                                               \
        }                                                                   \
    }                                                                       \
    {                                                                       \
...
//
//__global__ void spParticleBucketInitialize_kernel(dim3 start, dim3 count, dim3 strides, int num_pic,
//                                                  size_type *start_pos, size_type *f_count) {
//    uint x = __umul24(blockIdx.x, blockDim.x) + threadIdx.x;
//    uint y = __umul24(blockIdx.y, blockDim.y) + threadIdx.y;
//    uint z = __umul24(blockIdx.z, blockDim.z) + threadIdx.z;
//
//    if (x < count.x && y < count.y && z < count.z) {
//        uint s = __umul24(start.x + x, strides.x) + __umul24(start.y + y, strides.y) + __umul24(start.z + z,
//        strides.z);
...
//    SP_CALL(spMeshGetStrides(m, 0, m_strides));
//
...
//
//    SP_CALL_DEVICE_KERNEL(spParticleBucketInitialize_kernel, sizeType2Dim3(grid_dim), sizeType2Dim3(block_dim),
//                          sizeType2Dim3(m_start), sizeType2Dim3(m_count), sizeType2Dim3(m_strides), num_of_pic,
...
// */
//__global__ void spParticleBucketBuild_kernel(size_type *cellStart,                // output: cell start index
...
//}
//
//__global__ void _CopyBucketStartCount_kernel(size_type *b_start,  // output: cell start index
//                                             size_type *b_end,    // output: cell end index
//                                             size_type *start, size_type *count, size_type num_of_cell) {
...
//}
//
//__host__ int spParticleBucketBuild_device(spParticle *sp) {
...
//
//    SP_DEVICE_CALL_KERNEL2(spParticleBucketBuild_kernel, num_of_particle / NUMBER_OF_THREADS_PER_BLOCK + 1,
//                           NUMBER_OF_THREADS_PER_BLOCK, sMemSize, b_start, b_end, cell_hash, sorted_idx,
//                           num_of_particle, num_of_cell);
//
//    SP_CALL_DEVICE_KERNEL(_CopyBucketStartCount_kernel, num_of_cell / NUMBER_OF_THREADS_PER_BLOCK + 1,
//                          NUMBER_OF_THREADS_PER_BLOCK, b_start, b_end, bucket_start, bucket_count, num_of_cell);
//
...
//}
//
//__global__ void spParticleCoordinateConvert(particle_head *sp, Real3 dx, Real3 min, size_type const *start_pos,
//                                            size_type const *end_pos, size_type const *sorted_index) {
...
//    blockDim.z = 1;
//
//    SP_CALL_DEVICE_KERNEL(spParticleCoordinateConvert, sizeType2Dim3(dims), blockDim, (particle_head *)(p_data),
//                          real2Real3(dx), real2Real3(xmin), start_pos, end_pos, index);
//
//    return SP_SUCCESS;
//};
 ...
 */
__global__ void spRandomGeneratorSobolSetupKernel(unsigned long long *sobolDirectionVectors,
                                                  unsigned long long *sobolScrambleConstants, int num_of_dim,
                                                  size_type offset, struct curandStateScrambledSobol64 *state) {
    ...
    for (int i = 0; i < num_of_dim; ++i) {
        curand_init(sobolDirectionVectors + VECTOR_SIZE * (id * num_of_dim + i),
                    sobolScrambleConstants[id * num_of_dim + i], offset, &(state[id * num_of_dim + i]));
    }
}

__global__ void spRandomDistributionUniformKernel(struct curandStateScrambledSobol64 *state, Real *data,
                                                  size_type num) {
...
__global__ void spRandomDistributionNormalKernel(struct curandStateScrambledSobol64 *state, Real *data, size_type num) {
    unsigned int total_thread_id = threadIdx.x + __umul24(blockDim.x, blockIdx.x);
    unsigned int total_thread_num = __umul24(blockDim.x, gridDim.x);
    ...
    for (int n = 0; n < n_dof; ++n) {
        switch (dist_types[n]) {
            case SP_RAND_NORMAL:  //
                ...
                SP_CALL_DEVICE_KERNEL(spRandomDistributionNormalKernel, (n_threads / VECTOR_SIZE), VECTOR_SIZE,
                                      (devSobol64States + n * n_threads), data[n], num);

                break;
            case SP_RAND_UNIFORM:
            default:  //
                SP_CALL_DEVICE_KERNEL(spRandomDistributionUniformKernel, (n_threads / VECTOR_SIZE), VECTOR_SIZE,
                                      (devSobol64States + n * n_threads), data[n], num);
                break;
        }
    }
