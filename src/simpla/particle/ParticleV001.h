...
template<typename P, typename M> data_model::DataSet
Particle<P, M, V001>::dataset(mesh::EntityRange const &r0) const
{
...
//        Duplicate(r0, reinterpret_cast< value_type *>( ds.GetDataBlock.Serialize()));
    }
    return std::move(ds);
};
...
template<typename P, typename M> void
Particle<P, M, V001>::dataset(mesh::EntityRange const &r0, data_model::DataSet const &ds)
{
...
//OutputIT
//Particle<P, M, V001>::Duplicate(range_type const &r, OutputIT out_it) const
//{
//    //TODO need optimize
...
//         spNextBlank(&__it, m_pool_.Serialize()) != 0x0 && (ib != ie); ++ib)
//    {
...
//template<typename P, typename M> void
//Particle<P, M, V001>::Connect(mesh_id_type const &s, value_type const &v) { SetValue(s, &v, &v + 1); }
//
//
//template<typename P, typename M> template<typename Hash, typename TRange> void
//Particle<P, M, V001>::SetValue(Hash const &Hash, TRange const &v_r)
//{
...
//template<typename TInputIterator>
//void
//Particle<P, M, V001>::SetValue(mesh_id_type const &s, TInputIterator ib, TInputIterator ie)
//{
...
Particle<P, M, V001>::update(Args &&...args)
{
    ...
    __logger << "CMD:\t" << "GetPatch   [" << getClassName() << "]";
