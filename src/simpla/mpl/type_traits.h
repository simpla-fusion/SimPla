...
auto
invoke_helper(Func&& func, Tup&& tup, index_sequence<index...>) {
...
struct type_id {
   ...
   public:
    static std::string
    name() {
        return name_(std::integral_constant<
                     bool, has_static_member_function_class_name<T>::value>());
    }
};
...
struct check_static_bool_member_is_self_describing {
   private:
    ...
    static auto test(int,
                     std::enable_if_t<has_static_type_member_is_self_describing<
                         U, bool>::value>* = nullptr)
...
struct type_id<
    T, typename std::enable_if_t<
           detail::check_static_bool_member_is_self_describing<T>::value>> {
    static std::string
    name() {
        return T::name()();
    }
    ...
    static auto
    data_type() -> decltype(T::data_type()) {
        return T::data_type();
    }
};
...
//    {
//        return type_id<T[N]>::name() + "[" + traits::type_cast<int,
//        std::string>::eval(I) + "]";
//    }
//
//    static auto data_type() -> decltype(T::data_type()) { return
//    T::data_type(); }
//};
 ...
 * If T is '''is_indexable''' by '''S''', provides the member typedef type
 * equal to decltyp(T[S])
 * otherwise type is T. Note that if T is a multidimensional array, only the
 * first dimension is removed.
 */
...
struct remove_extent {
    ...
    template <typename _U>
    static auto test(int)
        -> decltype(std::declval<_U>()[std::declval<Idx>()]){};
...
struct remove_extents<T, I0, Others...> {
    typedef
        typename remove_extents<remove_extent_t<T, I0>, Others...>::type type;
};
 ...
 */
template <typename T, typename Idx = int>
struct rank : public index_const<(!is_indexable<T, Idx>::value)
                                     ? 0
...
remove_extents_t<T, I0, Idx...>&
get_v(T& v, I0 const& s0, Idx&&... idx) {
    return _detail::remove_entent_v<is_indexable<T, I0>::value>::get(
...
struct remove_entent_v<true> {
    template <typename T, typename I0>
    static remove_all_extents_t<T, I0>&
    get(T& v, I0 const* s) {
        return get_v(v[*s], s + 1);
    };

    template <typename T, typename I0, typename... Idx>
    static remove_extents_t<T, I0, Idx...>&
...
struct extent<integer_sequence<_Tp, N...>, 0>
    : public index_const<sizeof...(N)> {};

//**********************************************************************************************************************

// template<typename T>
// struct value_type { typedef typename
// std::conditional<std::is_scalar<T>::value, T, std::nullptr_t>::type type;
// };
//
// template<typename T> struct value_type<std::complex<T>> { typedef
// std::complex<T> type; };
//
// template<> struct value_type<std::string> { typedef std::string type; };
//
// template<typename _Tp, _Tp ...N> struct value_type<integer_sequence<_Tp,
// N...> > { typedef _Tp type; };
//
...
auto
max(T0 const& first, Others const&... others) {
...
template <typename T0, typename... Others>
auto
min(T0 const& first, Others const&... others) {
    return std::min(first, min(others...));
}

template <typename T>
auto
distance(T const& b, T const& e) {
    return e - b;
}

// template<typename T, typename TI>auto index(std::shared_ptr<T> &v, TI
// const &s) AUTO_RETURN(v.get()[s])
//
// template<typename T, typename TI>auto index(std::shared_ptr<T> const
// &v, TI const &s) AUTO_RETURN(v.get()[s])
...
// index(U const &v, TIndex const &i, ENABLE_IF(std::is_array<U>::vaule)) {
// return v[i]; };
//
// template<typename U, typename TIndex>
// U const &
// index(U const &v, TIndex const &i,
// ENABLE_IF(std::is_arithmetic<U>::vaule)) { return v; };
...
// template<typename T, typename TI>
// auto index(T &v, TI *s, ENABLE_IF((is_indexable<T, TI>::value)))
// AUTO_RETURN((_impl::recursive_try_index_aux<traits::rank<T>::value>::eval(v,
// s)))
//
// template<typename T, typename TI, size_type N>
// auto index(T &v, nTuple<TI, N> const &s, ENABLE_IF((is_indexable<T,
// TI>::value)))
...
//
//    template<typename U> static void set(T &v, U const &u) { v =
//    static_cast<T>(u); }
//};
//
// template<int N, typename ...T>
...
//    AUTO_RETURN((std::get<N>(v)))
//
//    static constexpr auto get(std::tuple<T...> const &v)
//    AUTO_RETURN((std::get<N>(v)))
//
//    template<typename U> static void set(std::tuple<T...> &v, U const &u)
//    { get(v) = u; }
//};
//
// template<int N, typename T>
...
//    AUTO_RETURN((std::get<N>(v)))
//
//    template<typename U> static void set(std::pair<T0, T1> &v, U const &u)
//    { get(v) = u; }
//};
////namespace _impl
////{
...
////    static void set(T &v, U const &u) { access_helper<N0, N...>::get(v)
///= u; }
////
////};
////
...
////{
////    template<typename T> static constexpr auto get(T &v)
/// AUTO_RETURN((access<N, T>::get(v)))
////
////    template<typename T> static constexpr auto get(T const &v)
/// AUTO_RETURN((access<N, T>::get(v)))
////
////    template<typename T, typename U> static void set(T &v, U const &u) {
/// access<N, T>::set(v, u); }
////
////};
////
...
////
////    template<typename T> static constexpr T const &get(T const &v) {
/// return v; }
////
////    template<typename T, typename U> static void set(T &v, U const &u) {
/// v = u; }
...
////};
////}  // namespace _impl
// template<int N, typename ...T> auto get(std::tuple<T...> &v)
// AUTO_RETURN((std::get<N>(v)))
//
...
T0
max(T0 const& first, T1 const& second) {
    return std::max(first, second);
};

template <typename T0, typename... O>
T0
max(T0 const& first, O&&... others) {
...
template <typename T0, typename T1>
T0
min(T0 const& first, T1 const& second) {
    return std::min(first, second);
};

template <typename T0, typename... Others>
T0
min(T0 const& first, Others&&... others) {
