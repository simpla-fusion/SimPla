...
auto invoke(Func&& func, Tup&& tup) AUTO_RETURN((_impl::invoke_helper(
    ...
    template <typename T>
    struct type_id<T, typename std::enable_if_t<
...
#undef DEFINE_TYPE_ID_NAME
     ...
     */
    template <typename T, typename Idx = int>
    struct rank : public index_const<
                      (!is_indexable<T, Idx>::value) ? 0 : 1 + rank<remove_extent<T, Idx>>::value> {
    };
    ...
    auto unpack_args(Args && ... args)
        AUTO_RETURN((_impl::unpack_args_helper<N>(std::forward<Args>(args)...)))
        ...
        struct recursive_try_index_aux {
            ...
            static auto eval(T& v, TI const* s)
                AUTO_RETURN((recursive_try_index_aux<N - 1>::eval(v[s[0]], s + 1)))
        };
