...
template<typename TRange> struct Iterator_;
template<typename TRange> struct ConstIterator_;
template<typename TRange> struct RangeHolder;
template<typename U, typename TRange> struct RangeProxy;

template<typename _Tp, typename _Category=std::input_iterator_tag, typename _Distance = ptrdiff_t, typename _Pointer = _Tp *, typename _Reference = _Tp &>
struct Range_
{
private:
...
public:
    ...
    template<typename U, typename ...Args> static this_type
    create(Args &&...args)
    {
        this_type res;
...
//    void append(this_type &&other) { m_holder_->append(other.m_holder_); }
    ...
    template<typename TFun> void foreach(TFun const &) const {}
...
public:
    ...
    template<typename U> U &as() { return *reinterpret_cast<U *>(data()); }
...
struct RangeProxy : public RangeHolder<TRange>
{
    ...
    {
        static_assert(traits::is_callable<TFun(value_type &)>::value, "Function is not  applicable! ");
