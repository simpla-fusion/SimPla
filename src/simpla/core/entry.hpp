...
#include "simpla/utils/factory.h"
#include "simpla/utils/logger.h"
#include "simpla/utils/object_head.h"
...
class Entry;
template <typename U>
size_type CopyFromData(U& dst, std::shared_ptr<const Entry> const& src);
template <typename U>
size_type CopyToData(std::shared_ptr<Entry> dst, U const& src);
template <typename U>
bool EqualTo(U const& dst, std::shared_ptr<const Entry> const& src);
...
class Entry : public std::enable_shared_from_this<Entry> {
   ...
   protected:
    ...
    std::shared_ptr<Entry> m_parent_ = nullptr;

   protected:
    explicit Entry(eNodeType etype = DN_TABLE);
    Entry(Entry const&);
    explicit Entry(std::shared_ptr<DataEntity> const&);
    explicit Entry(std::shared_ptr<const DataEntity> const&);

   public:
    virtual ~Entry();
    virtual std::shared_ptr<Entry> Copy() const;

    static std::shared_ptr<Entry> New(std::string const& uri = "");
    static std::shared_ptr<Entry> New(eNodeType e_type, std::string const& uri = "");
    static std::shared_ptr<Entry> New(std::shared_ptr<DataEntity> const& v);

    eNodeType type() const;
    std::shared_ptr<const Entry> GetRoot() const {
        return GetParent() == nullptr ? const_cast<Entry*>(this)->shared_from_this() : GetParent()->GetRoot();
    }
    std::shared_ptr<Entry> GetRoot() {
        return GetParent() == nullptr ? const_cast<Entry*>(this)->shared_from_this() : GetParent()->GetRoot();
    }
    bool isRoot() const { return GetParent() == nullptr; }
    void SetParent(std::shared_ptr<Entry> const& p) { m_parent_ = p; }
    std::shared_ptr<const Entry> GetParent() const { return m_parent_; }
    std::shared_ptr<Entry> GetParent() { return m_parent_; }

    /** @addtogroup required @{*/
    virtual std::shared_ptr<Entry> CreateNode(eNodeType e_type) const;
    virtual std::shared_ptr<Entry> CreateNode(std::string const& url, eNodeType e_type);
    ...
    virtual size_type Set(std::string const& uri, const std::shared_ptr<Entry>& v);
    virtual size_type Set(index_type s, const std::shared_ptr<Entry>& v);
    virtual size_type Set(const std::shared_ptr<Entry>& v);
    virtual size_type Set(std::string const& k, const std::shared_ptr<const Entry>& v) { return Set(k, v->Copy()); }
    virtual size_type Set(index_type k, const std::shared_ptr<const Entry>& v) { return Set(k, v->Copy()); }
    virtual size_type Set(const std::shared_ptr<const Entry>& v) { return Set(v->Copy()); }

    virtual size_type Add(index_type s, const std::shared_ptr<Entry>& v);
    virtual size_type Add(std::string const& uri, const std::shared_ptr<Entry>& v);
    virtual size_type Add(const std::shared_ptr<Entry>& v);
    virtual size_type Add(index_type k, const std::shared_ptr<const Entry>& v) { return Add(k, v->Copy()); }
    virtual size_type Add(std::string const& k, const std::shared_ptr<const Entry>& v) { return Add(k, v->Copy()); }
    virtual size_type Add(const std::shared_ptr<const Entry>& v) { return Add(v->Copy()); }
    virtual size_type Delete(std::string const& s);
    virtual std::shared_ptr<const Entry> Get(std::string const& uri) const;
    virtual std::shared_ptr<Entry> Get(std::string const& uri);

    virtual void Foreach(std::function<void(std::string const&, std::shared_ptr<const Entry> const&)> const& f) const;
    virtual void Foreach(std::function<void(std::string const&, std::shared_ptr<Entry> const&)> const& f);

    virtual size_type Delete(index_type s);
    virtual std::shared_ptr<const Entry> Get(index_type s) const;
    virtual std::shared_ptr<Entry> Get(index_type s);
    ...
    size_type SetValue(std::string const& s, Args&&... args) {
        return Set(s, Entry::New(DataLight::New(std::forward<Args>(args)...)));
    };
    template <typename U>
    size_type SetValue(std::string const& s, U const& v) {
        return Set(s, Entry::New(DataLight::New(v)));
    }
    template <typename U>
    ...
    size_type AddValue(std::string const& s, U const& u) {
        return Add(s, Entry::New(DataLight::New(u)));
    };

    template <typename U>
    size_type AddValue(std::string const& s, std::initializer_list<U> const& v) {
        return Add(s, Entry::New(DataLight::New(v)));
    }
    template <typename U>
...
struct KeyValue {
    ...
    KeyValue& operator=(U const& u) {
        m_node_ = Entry::New(DataLight::New(u));
        return *this;
    }
...
namespace detail {
template <typename U>
size_type _CopyFromData(U& dst, std::shared_ptr<const Entry> const& src, ENABLE_IF(std::rank<U>::value == 0)) {
...
template <typename U>
size_type _CopyToData(std::shared_ptr<Entry> dst, U const& src, ENABLE_IF(std::rank<U>::value == 1)) {
...
template <typename U>
size_type _CopyToData(std::shared_ptr<Entry> dst, U const& src, ENABLE_IF((std::rank<U>::value > 0))) {
    if (dst == nullptr) { return 0; }
    size_type count = 0;
    static auto snan = std::numeric_limits<traits::value_type_t<U>>::signaling_NaN();
...
template <typename U>
bool _CompareToData(U const& dst, std::shared_ptr<const Entry> const& src, ENABLE_IF(std::rank<U>::value == 0)) {
...
template <typename U>
bool _CompareToData(U const& dst, std::shared_ptr<Entry> const& src, ENABLE_IF((std::rank<U>::value > 0))) {
