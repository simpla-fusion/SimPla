...
Configurable::Configurable() : m_db_(data::Entry::New(data::Entry::DN_TABLE)){};
...
void Configurable::SetDB(std::shared_ptr<Entry> const &cfg) {
    m_db_ = cfg;
    if (m_db_ != nullptr) {
        Push();
        Pop();
...
void Configurable::SetDB(std::shared_ptr<const Entry> const &cfg) {
    if (cfg != nullptr) {
        if (m_db_ == nullptr) { m_db_ = data::Entry::New(data::Entry::DN_TABLE); }
        m_db_->Set(cfg);
        Push();
        Pop();
    }
