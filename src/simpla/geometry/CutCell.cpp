...
void CutCellTagNodeSimPla(Array<unsigned int> *node_tags, Chart const &chart, index_box_type const &idx_box,
                          const GeoObject &g, int tag) {
    if (auto box = dynamic_cast<const Box *>(&g)) {
...
void CutCellTagNode(Array<unsigned int> *vertex_tags, Chart const &chart, index_box_type const &idx_box,
                    const GeoObject &g, unsigned int tag) {
    if (false) {
    }
#ifdef OCE_FOUND
