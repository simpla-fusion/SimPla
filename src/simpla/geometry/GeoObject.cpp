...
struct GeoObjectFactory::pimpl_s {
    std::map<std::string, std::function<GeoObject *(std::shared_ptr<data::DataEntity> const &)>> m_mesh_factory_;
};
...
bool GeoObjectFactory::RegisterCreator(
    std::string const &k, std::function<GeoObject *(std::shared_ptr<data::DataEntity> const &)> const &fun) {
    return m_pimpl_->m_mesh_factory_.emplace(k, fun).second;
};

GeoObject *GeoObjectFactory::Create(std::shared_ptr<data::DataEntity> const &t) const {
    GeoObject *res = nullptr;
    if (t == nullptr) {
        res = new Cube(box_type{{0, 0, 0}, {1, 1, 1}});
