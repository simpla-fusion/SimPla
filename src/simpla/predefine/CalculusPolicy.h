...
struct InterpolatePolicy {
   ...
   private:
    ...
    static U const& eval(declare::Field_<U, M, I...>& f,
                         MeshEntityId const& s) {
        return f[s];
    };

    template <typename U, typename M, size_type... I>
    static U& eval(declare::Field_<U, M, I...> const& f,
                   MeshEntityId const& s) {
        return f[s];
    };
   ...
   public:
    ...
    constexpr static traits::field_value_t<TF> gather(
        mesh_type const& m, TF const& f, point_type const& r,
    ...
    template <typename TF>
    constexpr static traits::field_value_t<TF> gather(
        mesh_type const& m, TF const& f, point_type const& x,
   ...
   private:
    ...
    static void scatter_(mesh_type const& m, index_const<VERTEX>, TF& f,
                         TX const& x, TV const& u) {
        scatter_impl_(f, m.point_global_to_local(x, 0), u);
    }

    template <typename TF, typename TX, typename TV>
    static void scatter_(mesh_type const& m, index_const<EDGE>, TF& f,
                         TX const& x, TV const& u) {
    ...
    template <typename TF, typename TX, typename TV>
    static void scatter_(mesh_type const& m, index_const<FACE>, TF& f,
                         TX const& x, TV const& u) {
    ...
    template <typename TF, typename TX, typename TV>
    static void scatter_(mesh_type const& m, index_const<VOLUME>, TF& f,
                         TX const& x, TV const& u) {
   ...
   private:
    ...
    static decltype(auto) sample_(mesh_type const& m, index_const<I>,
                                  MeshEntityId const& s, TV& v) {
        return v;
    }

    template <typename TV, size_type L>
    static decltype(auto) sample_(mesh_type const& m, index_const<VERTEX>,
                                  MeshEntityId const& s,
                                  nTuple<TV, L> const& v) {
        return v[s.w % L];
    }
    ...
    template <typename TV, size_type L>
    static decltype(auto) sample_(mesh_type const& m, index_const<VOLUME>,
                                  MeshEntityId const& s,
                                  nTuple<TV, L> const& v) {
        return v[s.w % L];
    }
    ...
    static decltype(auto) sample_(mesh_type const& m, index_const<EDGE>,
                                  MeshEntityId const& s,
                                  nTuple<TV, 3> const& v) {
    ...
    template <typename TV>
    static decltype(auto) sample_(mesh_type const& m, index_const<FACE>,
                                  MeshEntityId const& s,
                                  nTuple<TV, 3> const& v) {
        return v[M::sub_index(s)];
    }
   ...
   public:
    ...
    Real RBF(mesh_type const& m, point_type const& x0, point_type const& x1,
        ...
        return (1.0 - std::abs(r[0])) * (1.0 - std::abs(r[1])) *
               (1.0 - std::abs(r[2]));
    }
    ...
    static void assign(declare::Field_<V, mesh_type, IFORM, DOF>& f,
    ...
    static void assign(declare::Field_<V, mesh_type, EDGE, DOF>& f,
                       mesh_type const& m, MeshEntityId const& s,
                       nTuple<U, 3> const& v) {
        for (int i = 0; i < DOF; ++i) {
    ...
    template <typename V, size_type DOF, typename U>
    static void assign(declare::Field_<V, mesh_type, FACE, DOF>& f,
                       mesh_type const& m, MeshEntityId const& s,
                       nTuple<U, 3> const& v) {
        for (int i = 0; i < DOF; ++i) {
            f[M::sw(s, i)] = v[M::sub_index(s)];
    ...
    template <typename V, size_type DOF, typename U>
    static void assign(declare::Field_<V, mesh_type, VOLUME, DOF>& f,
                       mesh_type const& m, MeshEntityId const& s,
                       nTuple<U, DOF> const& v) {
        for (int i = 0; i < DOF; ++i) {
            f[M::sw(s, i)] = v[i];
    ...
    template <typename V, mesh::MeshEntityType IFORM, size_type DOF, typename U>
    static void assign(declare::Field_<V, mesh_type, IFORM, DOF>& f,
                       mesh_type const& m, MeshEntityId const& s, U const& v) {
        for (int i = 0; i < DOF; ++i) {
            f[M::sw(s, i)] = v;
        }
    }
...
struct calculator<algebra::declare::Field_<TV, TM, IFORM, DOF>> {
   ...
   private:
    ...
    static decltype(auto) get_v(mesh_type const& m, FExpr const& f,
                                MeshEntityId const s) {
        return get_value(m, f, s) * m.volume(s);
    }

    template <typename FExpr>
    static decltype(auto) get_d(mesh_type const& m, FExpr const& f,
                                MeshEntityId const s) {
    ...
    static decltype(auto) eval(
        mesh_type const& m, TExpr const& expr, MeshEntityId const& s,
    ...
    template <typename TExpr>
    static decltype(auto) eval(mesh_type const& m, TExpr const& expr,
                               MeshEntityId const& s,
   ...
   private:
    ...
    static typename traits::value_type<T>::type _map_to(mesh_type const& m,
                                                        T const& f,
                                                        MeshEntityId const& s,
                                                        index_sequence<I, I>) {
        return f[s];
    ...
    template <typename TExpr>
    static decltype(auto) _map_to(mesh_type const& m, TExpr const& expr,
                                  MeshEntityId const& s,
        ...
        return (l + r) * 0.5;
        //        return (get_value(m, expr, sw(s - X, n)) + get_value(m, expr,
        //        sw(s
        //        + X, n))) * 0.5;
    }
    ...
    static decltype(auto) _map_to(mesh_type const& m, TExpr const& expr,
        ...
        //                        traits::value_type<TF>::type>(get_value(m, l,
        //                        (s -
        //                        Y - Z)) +
        //                                                                           get_value(m, l, (s - Y + Z)) +
        //                                                                           get_value(m, l, (s + Y - Z)) +
    ...
    static decltype(auto) eval(mesh_type const& m, TExpr const& expr,
        ...
        return m.inner_product(_map_to(m, std::get<0>(expr.m_args_), s,
                                       index_sequence<IL, IR + IL>()),
                               _map_to(m, std::get<1>(expr.m_args_), s,
                                       index_sequence<IR, IR + IL>()),
                               s);
    }
    ...
    static decltype(auto) eval(mesh_type const& m, TExpr const& expr,
        ...
        AUTO_RETURN((get_value(m, std::get<0>(expr.m_args_), sw(s, 0)) *
        ...
        static decltype(auto)
            _invoke_helper(mesh_type const& m, TExpr const& expr,
                           MeshEntityId const& s,
                           index_sequence<I...>)  //
        AUTO_RETURN((expr.m_op_(get_value(m, std::get<I>(expr.m_args_), s)...)))

            template <typename TExpr, typename TOP, size_type... I>
            static decltype(auto)
                eval(mesh_type const& m, TExpr const& expr,
                     MeshEntityId const& s, expression_tag<TOP, I...>)  //
        AUTO_RETURN((_invoke_helper(m, expr, s,
    ...
    static decltype(auto) get_value(mesh_type const& m,
    ...
    static decltype(auto) get_value(mesh_type const& m, data_block_type& d,
                                    MeshEntityId const& s) {
        auto idx = M::unpack_index4(s);
        return d.at(&idx[0]);
    };
    ...
    template <typename TOP, typename... T>
    static decltype(auto) get_value(mesh_type const& m,
                                    declare::Expression<TOP, T...> const& expr,
                                    MeshEntityId const& s) {
        return eval(m, expr, s,
                    expression_tag<TOP, algebra::traits::iform<T>::value...>());
    }

    /*@formatter:off*/
    template <typename TFun>
    static decltype(auto) get_value(
        mesh_type const& m, TFun const& fun, MeshEntityId const& s,
        ENABLE_IF((st::is_callable<TFun(MeshEntityId const&)>::value)))  //
        AUTO_RETURN((
            InterpolatePolicy<mesh_type>::template sample<IFORM>(m, s, fun(s))))
        ...
        AUTO_RETURN((InterpolatePolicy<mesh_type>::template sample<IFORM>(
            m, s, fun(m.point(s)))))
        ...
        r.foreach ([&](mesh::MeshEntityId const& s) {
            op(get_value(m, self, s),
               get_value(m, std::forward<Args>(args), s)...);
        });
    }
};
