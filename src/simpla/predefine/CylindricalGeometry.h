...
public:
  ...
  Variable<Real, VOLUME, 9> m_inv_dual_volume_{this,
                                               "name=inv_dual_volume;NO_FILL"};
...
public:
  ...
  virtual point_type point(index_type i, index_type j, index_type k) const {
    return point_type{m_vertics_(i, j, k, 0), m_vertics_(i, j, k, 1),
                      m_vertics_(i, j, k, 2)};
  };

  virtual point_type point(MeshEntityId s) const {
    return m_mesh_block_->point(s);
  };
  ...
  virtual Real volume(MeshEntityId s) const {
    return m_volume_[mesh_block()->hash(s)];
  }
