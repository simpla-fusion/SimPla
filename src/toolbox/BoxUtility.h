...
dimensions(Box3<T> const &b)
{
    return
            Vector3<T>
                    {
                            static_cast<typename std::make_signed<T>::type>(std::get<1>(b)[0]) - std::get<0>(b)[0],
                            static_cast<typename std::make_signed<T>::type>(std::get<1>(b)[1]) - std::get<0>(b)[1],
                            static_cast<typename std::make_signed<T>::type>(std::get<1>(b)[2]) - std::get<0>(b)[2]
                    };
}


template<typename U, typename V> constexpr inline Point3<U>
convert(Point3<V> const &b) { return Point3<U>{b[0], b[1], b[2]}; }
