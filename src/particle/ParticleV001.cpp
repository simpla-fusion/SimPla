            ...
            r0, [&](range_type const &r)
            {
                ...
                for (auto const &key:r)
                {
                    ...
                    while (num_of_element > 0)
                    {
                        ...
                        {
                            if (reinterpret_cast<_point_s *>(src)->_cell_id != key)
                            {
                                memcpy(__it.p, src, size_in_byte);
                                tag <<= 1;
                                dest += size_in_byte;
                                pg->tag |= tag;
                            }
                    ...
                    if (m_data_->insert(accessor, key)) { sp::spPushFront(&(accessor->second), &pg); }
                }
            }
    );
}
