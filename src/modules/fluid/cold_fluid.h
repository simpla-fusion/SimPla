	...
	BOOST_FOREACH( const typename ptree::value_type &v, pt)
	{
		if (v.first == "Compound")
		{
			boost::optional<TR1::shared_ptr<Object> > obj =
					ctx.objects[v.second.get_value<std::string>()];
...
//			E + (Curl(B) / mu0 - J) / epsilon0 * (dt * 0.5));
	...
	for (typename std::list<TR1::shared_ptr<CompoundObject> >::iterator it =
			sp_list.begin(); it != sp_list.end(); ++it)
	{
		ZeroForm & ns = *TR1::dynamic_pointer_cast<ZeroForm>(
				(*it)->objects["n"]);
		VecZeroForm & Js = *TR1::dynamic_pointer_cast<VecZeroForm>(
				(*it)->objects["J"]);
		Real ms = (*it)->properties.get<Real>("m") * proton_mass;
		Real Zs = (*it)->properties.get<Real>("Z") * elementary_charge;
