...
public:
    ...
    typedef typename mesh_type::template field<VERTEX, nTuple < Real, 3> >
    E0_type;
...
void Particle<TM, ColdFluid, PolicyFluidParticle>::next_timestep_half(
        ...
        B1_type const &B1)
{
    LOGGER << "Push m_fluid_sp_ Step Half[ " << get_type_as_string() << "]";
...
{
    LOGGER << "Push m_fluid_sp_ sync fields[ " << get_type_as_string() << "]";
