...
void spBorisYeeInitializeParticle(spParticle *sp, int NUM_OF_PIC)
{
    ...
    ADD_PARTICLE_ATTRIBUTE(sp, boris_s, Real, vx);
    ADD_PARTICLE_ATTRIBUTE(sp, boris_s, Real, vy);
    ADD_PARTICLE_ATTRIBUTE(sp, boris_s, Real, vz);
    ADD_PARTICLE_ATTRIBUTE(sp, boris_s, Real, f);
    ADD_PARTICLE_ATTRIBUTE(sp, boris_s, Real, w);
...
MC_DEVICE void
spUpdateParticleBorisPushThreadKernel(int s0, int sn, spPage const *pg, spPage **dest, MeshEntityId tag, const Real *tE,
                                      const Real *tB,
                                      Real3 inv_dv, Real cmr_dt)
{

    MC_SHARED int dest_tail;

    for (int s = s0; s < SP_NUMBER_OF_ENTITIES_IN_PAGE; s += sn)
    {
        MeshEntityId id = P_GET(pg->data, struct boris_s, MeshEntityId, id, s);
        ...
        /*    @formatter:on */
        P_GET(pg->data, struct boris_s, MeshEntityId, id, s) = id;
        P_GET(pg->data, struct boris_s, Real, rx, s) = rx;
        P_GET(pg->data, struct boris_s, Real, ry, s) = ry;
        P_GET(pg->data, struct boris_s, Real, rz, s) = rz;
        P_GET(pg->data, struct boris_s, Real, vx, s) = vx;
        P_GET(pg->data, struct boris_s, Real, vy, s) = vy;
        P_GET(pg->data, struct boris_s, Real, vz, s) = vz;
...
MC_GLOBAL void
spUpdateParticleBorisPushBlockKernel(spPage **buckets, const Real *fE, const Real *fB, Real3 inv_dv,
                                     Real cmr_dt)
{
...
MC_GLOBAL void spUpdateParticleBorisScatterBlockKernel(spPage **buckets, Real *fRho, Real *fJ)
{
   ...
   /*    @formatter:on */
        ...
        spAtomicAdd(&(fJ[BLOCK_ID + SP_MESH_NUM_OF_ENTITY_IN_GRID * 0]), J4.x);
        spAtomicAdd(&(fJ[BLOCK_ID + SP_MESH_NUM_OF_ENTITY_IN_GRID * 1]), J4.y);
        spAtomicAdd(&(fJ[BLOCK_ID + SP_MESH_NUM_OF_ENTITY_IN_GRID * 2]), J4.z);
        spAtomicAdd(&(fRho[BLOCK_ID]), J4.w);
    }
...
void spBorisYeeUpdateParticle(spParticle *sp, Real dt, const spField *fE, const spField *fB, spField *fRho, spField *fJ)
{
    ...
    /*    @formatter:off */
    spUpdateParticleBorisPushBlockKernel<<< sp->m->dims, NUMBER_OF_THREADS_PER_BLOCK >>>(sp->buckets, ( fE->device_data),
                                                          (  fB->device_data,inv_dv,cmr_dt));

    spUpdateParticleBorisSortBlockKernel<<<sp->m->dims, NUMBER_OF_THREADS_PER_BLOCK >>>(sp->buckets);
